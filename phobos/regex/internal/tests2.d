...
module stdd.regex.internal.tests2;

package(stdd.regex):

import stdd.conv, stdd.exception, stdd.meta, stdd.range,
    stdd.typecons, stdd.regex;

import stdd.uni : Escapables; // characters that need escaping
...
//tests for accumulated stdd.regex issues and other regressions
@safe unittest
{
    import stdd.algorithm.comparison : equal;
    import stdd.algorithm.iteration : map;
...
@safe unittest
{
    void test(alias matchFn)()
    {
        ...
        {{
            ...
            alias String = v;
            assert(stdd.regex.replace!(matchFn)(to!String("ark rapacity"), regex(to!String("r")), to!String("c"))
                   == to!String("ack rapacity"));
            assert(stdd.regex.replace!(matchFn)(to!String("ark rapacity"), regex(to!String("r"), "g"), to!String("c"))
                   == to!String("ack capacity"));
            assert(stdd.regex.replace!(matchFn)(to!String("noon"), regex(to!String("^n")), to!String("[$&]"))
                   == to!String("[n]oon"));
            assert(stdd.regex.replace!(matchFn)(
                to!String("test1 test2"), regex(to!String(`\w+`),"g"), to!String("$`:$'")
            ) == to!String(": test2 test1 :"));
            auto s = stdd.regex.replace!(baz!(Captures!(String)))(to!String("Strap a rocket engine on a chicken."),
...
{// bugzilla 7679
    ...
    {{
        ...
        auto str = to!S("a.b");
        assert(equal(stdd.regex.splitter(str, re), [to!S("a"), to!S("b")]));
...
@safe unittest
{
    import stdd.datetime.stopwatch : StopWatch, AutoStart;
...
@safe unittest
{
    ...
    import stdd.regex : replaceFirst, replaceFirstInto, regex;
