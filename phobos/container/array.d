 ...
 *
 * Source: $(PHOBOSSRC stdd/container/_array.d)
 *
 ...
 * Note:
 *
 * When using `Array` with range-based functions like those in `stdd.algorithm`,
...
if (!is(Unqual!T == bool))
{
    ...
    private struct Payload
    {
        ...
        {
            import stdd.algorithm.mutation : initializeAll;
        ...
        void reserve(size_t elements)
        {
            ...
            static if (hasIndirections!T)
            {
                ...
                newPayloadPtr || assert(false, "stdd.container.Array.reserve failed to allocate memory");
    ...
    void reserve(size_t elements)
    {
        if (!_data.refCountedStore.isInitialized)
        {
            ...
            if (overflow) assert(0);
            auto p = malloc(sz);
            p || assert(false, "stdd.container.Array.reserve failed to allocate memory");
            static if (hasIndirections!T)
            {
                GC.addRange(p, sz);
            }
...
@safe unittest
{
    ...
    import stdd.container : Array, BinaryHeap;
