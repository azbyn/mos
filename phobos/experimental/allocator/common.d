...
/**
Utility and ancillary artifacts of `stdd.experimental.allocator`. This module
shouldn't be used directly; its functionality will be migrated into more
appropriate parts of `stdd`.

Authors: $(HTTP erdani.com, Andrei Alexandrescu), Timon Gehr (`Ternary`)

Source: $(PHOBOSSRC stdd/experimental/allocator/_common.d)
*/
module stdd.experimental.allocator.common;
import stdd.algorithm.comparison, stdd.traits;
...
current platform.
*/
enum uint platformAlignment = stdd.algorithm.comparison.max(double.alignof, real.alignof);
...
{
    string result = "    import stdd.traits : hasMember, Parameters;\n";
    foreach (fun; funs)
    {
        result ~= "
...
version(unittest)
{
    import stdd.experimental.allocator : RCIAllocator, RCISharedAllocator;

    package void testAllocator(alias make)()
    {
        import stdd.conv : text;
        import stdd.math : isPowerOf2;
        import stdd.stdio : writeln, stderr;
        import stdd.typecons : Ternary;
        alias A = typeof(make());
