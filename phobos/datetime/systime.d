...
$(TR $(TD Conversion) $(TD
    ...
    Source:    $(PHOBOSSRC stdd/datetime/_systime.d)
+/
module stdd.datetime.systime;
...
import stdd.datetime.date;// : _monthNames, AllowDayOverflow, CmpTimeUnits, Date,
...
import stdd.datetime.timezone;// : LocalTime, SimpleTimeZone, TimeZone, UTC;
import stdd.exception : enforce;
import stdd.format : format;
import stdd.range.primitives;
import stdd.traits : isIntegral, isSigned, isSomeString, Unqual, isNarrowString;
...
public:
    ...
    /++
        ...
        Throws:
            $(REF DateTimeException,stdd,datetime,date) if it fails to get the
            time.
      +/
    ...
    static @property long currStdTime(ClockType clockType = ClockType.normal)() @trusted
    {
        ...
        version(Windows)
        {
            ...
            {
                // Ideally, this would use core.stdd.time.time, but the C runtime
...
/++
    ...
    $(REF DateTime,stdd,datetime,date), the time zone is an integral part of
    ...
    its internal time in stdd time (hnsecs since midnight, January 1st, 1 A.D.
    UTC), so it interfaces well with the system time. However, that means that,
    unlike $(REF DateTime,stdd,datetime,date), it is not optimized for
    ...
    care about time zones, then $(REF DateTime,stdd,datetime,date) would be
    the type to use. For system time, use $(D SysTime).

    $(LREF Clock.currTime) will return the current time as a $(D SysTime).
    To convert a $(D SysTime) to a $(REF Date,stdd,datetime,date) or
    $(REF DateTime,stdd,datetime,date), simply cast it. To convert a
    $(REF Date,stdd,datetime,date) or $(REF DateTime,stdd,datetime,date) to a
    $(D SysTime), use $(D SysTime)'s constructor, and pass in the ntended time
    zone with it (or don't pass in a $(REF TimeZone,stdd,datetime,timezone), and
    the local time zone will be used). Be aware, however, that converting from a
    $(REF DateTime,stdd,datetime,date) to a $(D SysTime) will not necessarily
    ...
    $(REF PosixTimeZone,stdd,datetime,timezone) on Posix systems (or on Windows,
    if providing the TZ Database files), and use
    $(REF WindowsTimeZone,stdd,datetime,timezone) on Windows systems. The time in
    ...
    $(D SysTime). $(REF LocalTime,stdd,datetime,timezone) is the
    $(REF TimeZone,stdd,datetime,timezone) class which represents the local time,
    and $(D UTC) is the $(REF TimeZone,stdd,datetime,timezone) class which
    represents UTC. $(D SysTime) uses $(REF LocalTime,stdd,datetime,timezone) if
    no $(REF TimeZone,stdd,datetime,timezone) is provided. For more details on
    time zones, see the documentation for $(REF TimeZone,stdd,datetime,timezone),
    $(REF PosixTimeZone,stdd,datetime,timezone), and
    $(REF WindowsTimeZone,stdd,datetime,timezone).
...
struct SysTime
{
    ...
    import stdd.typecons : Rebindable;

public:

    /++
        Params:
            dateTime = The $(REF DateTime,stdd,datetime,date) to use to set
                       this $(LREF SysTime)'s internal stdd time. As
                       $(REF DateTime,stdd,datetime,date) has no concept of
                       time zone, tz is used as its time zone.
            tz       = The $(REF TimeZone,stdd,datetime,timezone) to use for this
                       $(LREF SysTime). If null,
                       $(REF LocalTime,stdd,datetime,timezone) will be used. The
                       given $(REF DateTime,stdd,datetime,date) is assumed to
    ...
    /++
        ...
        Throws:
            $(REF DateTimeException,stdd,datetime,date) if $(D fracSecs) is negative or if it's
    ...
    /++
        Params:
            date = The $(REF Date,stdd,datetime,date) to use to set this
                   $(LREF SysTime)'s internal stdd time. As
                   $(REF Date,stdd,datetime,date) has no concept of time zone, tz
                   is used as its time zone.
            tz   = The $(REF TimeZone,stdd,datetime,timezone) to use for this
                   $(LREF SysTime). If null,
                   $(REF LocalTime,stdd,datetime,timezone) will be used. The
                   given $(REF Date,stdd,datetime,date) is assumed to be in the
                   given time zone.
      +/
    ...
    /++
        Note:
            ...
            since midnight, January 1st, 1 A.D. UTC - i.e. stdd time - this
            constructor takes a stdd time, which is specifically already in UTC,
        ...
        Params:
            ...
            tz      = The $(REF TimeZone,stdd,datetime,timezone) to use for this
                      $(LREF SysTime). If null,
                      $(REF LocalTime,stdd,datetime,timezone) will be used.
      +/
    ...
    /++
        ...
        stdd times (which are in UTC) are compared.
     +/
    ...
    /++
        ...
        Throws:
            $(REF DateTimeException,stdd,datetime,date) if the new year is not
    ...
    @safe unittest
    {
        import stdd.datetime.date : DateTime;
    ...
    /++
        ...
        Throws:
            $(REF DateTimeException,stdd,datetime,date) if $(D isAD) is true.
     +/
    ...
    @safe unittest
    {
        import stdd.exception : assertNotThrown;
    ...
    /++
        ...
        Throws:
            $(REF DateTimeException,stdd,datetime,date) if a non-positive value
            is given.
     +/
    ...
    /++
        ...
        Throws:
            $(REF DateTimeException,stdd,datetime,date) if the given month is
            not a valid month.
     +/
    ...
    /++
        ...
        Throws:
            $(REF DateTimeException,stdd,datetime,date) if the given day is not
    ...
    {
        import stdd.range : chain;
        import stdd.traits : EnumMembers;
    ...
    /++
        ...
        Throws:
            $(REF DateTimeException,stdd,datetime,date) if the given hour are
    ...
    /++
        ...
        Throws:
            $(REF DateTimeException,stdd,datetime,date) if the given minute are
    ...
    /++
        ...
        Throws:
            $(REF DateTimeException,stdd,datetime,date) if the given second are
    ...
    /++
        ...
        Throws:
            $(REF DateTimeException,stdd,datetime,date) if the given duration
    ...
    /++
        ...
        Params:
            timezone = The $(REF _TimeZone,stdd,datetime,_timezone) to set this
    ...
    /++
        Returns a $(LREF SysTime) with the same stdd time as this one, but with
        $(REF LocalTime,stdd,datetime,timezone) as its time zone.
      +/
    ...
    @safe unittest
    {
        ...
        import stdd.datetime.date : DateTime;
        import stdd.datetime.timezone : SimpleTimeZone, UTC;
    ...
    /++
        ...
        Note that like all conversions in stdd.datetime, this is a truncating
        conversion.
    ...
    @system unittest
    {
        ...
        version(Posix)
        {
            import stdd.datetime.timezone : clearTZEnvVar, setTZEnvVar;
    ...
    @safe unittest
    {
        import stdd.datetime.date : AllowDayOverflow, DateTime;
    ...
    @safe unittest
    {
        ...
        version(Posix)
        {
            import stdd.datetime.timezone : PosixTimeZone;
            immutable tz = PosixTimeZone.getTimeZone("America/Los_Angeles");
        }
        else version(Windows)
        {
            import stdd.datetime.timezone : WindowsTimeZone;
    ...
    {
        import core.time;
        import stdd.datetime.date : Date;
    ...
    /++
        Returns a $(REF Date,stdd,datetime,date) equivalent to this $(LREF SysTime).
      +/
    ...
    /++
        Returns a $(REF DateTime,stdd,datetime,date) equivalent to this
        $(LREF SysTime).
      +/
    ...
    /++
        Returns a $(REF TimeOfDay,stdd,datetime,date) equivalent to this
        $(LREF SysTime).
      +/
    ...
    /++
        ...
        $(REF LocalTime,stdd,datetime,timezone), then TZ is empty. If its time
        ...
        $(RED Warning:
            ...
            $(REF LocalTime,stdd,datetime,timezone) or
            $(REF UTC,stdd,datetime,timezone), which is not in conformance with
    ...
    /++
        ...
        $(REF LocalTime,stdd,datetime,timezone) is used. If the time zone is "Z",
        then $(D UTC) is used. Otherwise, a
        $(REF SimpleTimeZone,stdd,datetime,timezone) which corresponds to the
        ...
        $(RED Warning:
            ...
            zone when it was not $(REF LocalTime,stdd,datetime,timezone) or
            $(REF UTC,stdd,datetime,timezone), which is not in conformance with
            ISO 8601 for the non-extended string format. This has now been
            fixed. However, for now, fromISOString will continue to accept the
            extended format for the time zone so that any code which has been
        ...
        Throws:
            $(REF DateTimeException,stdd,datetime,date) if the given string is
    ...
    static SysTime fromISOString(S)(in S isoString, immutable TimeZone tz = null) @safe
        if (isSomeString!S)
    {
        import stdd.algorithm.searching : startsWith, find;
        import stdd.conv : to;
        import stdd.string : strip;
    ...
    static SysTime fromISOExtString(S)(in S isoExtString, immutable TimeZone tz = null) @safe
        if (isSomeString!(S))
    {
        import stdd.algorithm.searching : countUntil, find;
        import stdd.conv : to;
        import stdd.string : strip, indexOf;
...
private:
    ...
    /+
        Converts the given hnsecs from $(LREF SysTime)'s time zone to stdd time.
      +/
...
/++
    ...
    epoch and seconds as its units) to "stdd time" (which uses midnight,
    ...
    "stdd time"'s epoch is based on the Proleptic Gregorian Calendar per ISO
    ...
    the standard, much as it's based on it, so the name "stdd time" isn't
    ...
    stdd.datetime uses the term "stdd time" for this.
...
{
    import stdd.datetime.date : DateTime;
    import stdd.datetime.timezone : UTC;
...
/++
    Converts stdd time (which uses midnight, January 1st, 1 A.D. UTC as its epoch
    ...
    Params:
        T = The return type (int or long). It defaults to time_t, which is
            normally 32 bits on a 32-bit system and 64 bits on a 64-bit
            system.
        stdTime = The stdd time to convert.
...
if (is(T == int) || is(T == long))
{
    ...
    static assert(is(time_t == int) || is(time_t == long),
                  "Currently, stdd.datetime only supports systems where time_t is int or long");
...
version(StdDdoc)
{
    ...
    /++
        ...
        Throws:
            $(REF DateTimeException,stdd,datetime,date) if the given
    ...
    /++
        ...
        Throws:
            $(REF DateTimeException,stdd,datetime,date) if the given value will
...
/++
    ...
    Throws:
        $(REF DateTimeException,stdd,datetime,date) if the $(D DosFileTime) is
        invalid.
  +/
...
/++
    ...
    $(REF SimpleTimeZone,stdd,datetime,timezone) with a utc offset of $(D 0) is
    used rather than $(REF UTC,stdd,datetime,timezone), whereas $(D "+0000") uses
    $(REF UTC,stdd,datetime,timezone).
    ...
    Throws:
        $(REF DateTimeException,stdd,datetime,date) if the given string doesn't
...
{
    import stdd.string : representation;
...
{
    import stdd.algorithm.searching : find, all;
    import stdd.ascii : isDigit, isAlpha, isPrintable;
    import stdd.conv : to;
    import stdd.functional : not;
    import stdd.string : capitalize, format;
    import stdd.traits : EnumMembers, isArray;
    import stdd.typecons : Rebindable;
...
afterMon: stripAndCheckLen(value[3 .. value.length], "1200:00A".length);
    ...
    // year
    auto found = value[2 .. value.length].find!(not!(stdd.ascii.isDigit))();
    ...
    else
    {
        ...
        switch (sliceAsString(value[0 .. tzLen <= 4 ? tzLen : 4]))
        {
            ...
            default:
            {
                if (all!(stdd.ascii.isAlpha)(value[0 .. tzLen]))
                {
...
{
    import core.time : hours;
    import stdd.datetime.date : DateTime, DateTimeException;
    import stdd.datetime.timezone : SimpleTimeZone, UTC;
    import stdd.exception : assertThrown;
...
{
    import core.time;
    import stdd.algorithm.iteration : filter, map;
    import stdd.algorithm.searching : canFind;
    import stdd.array : array;
    import stdd.ascii : letters;
    import stdd.format : format;
    import stdd.meta : AliasSeq;
    import stdd.range : chain, iota, take;
    import stdd.stdio : writefln, writeln;
    import stdd.string : representation;
    ...
    static struct Rand3Letters
    {
        ...
        void popFront()
        {
            import stdd.exception : assumeUnique;
            import stdd.random : rndGen;
    ...
    {{
        ...
        // dst and stdd times are switched in the Southern Hemisphere which is why the
        ...
        import stdd.uni : toLower, toUpper;
...
@system unittest
{
    import stdd.algorithm.iteration : filter, map;
    import stdd.ascii : letters;
    import stdd.exception : collectExceptionMsg;
    import stdd.format : format;
    import stdd.meta : AliasSeq;
...
{
    import stdd.conv : toChars;
    import stdd.range : padLeft;
...
if (isSomeString!S)
{
    import stdd.algorithm.searching : all;
...
// This is so that we don't have to worry about stdd.conv.to throwing. It also
// doesn't have to worry about quite as many cases as stdd.conv.to, since it
