...
/++
    ...
    $(UL
        $(LI Types to represent points in time:
             $(REF SysTime,stdd,_datetime,systime),
             $(REF Date,stdd,_datetime,date),
             $(REF TimeOfDay,stdd,_datetime,date),
             $(REF DateTime,stdd,_datetime,date).)
    ...
    Closely related to stdd.datetime is <a href="core_time.html">$(D core.time)</a>,
    and some of the time types used in stdd.datetime come from there - such as
    $(REF Duration, core,time), $(REF TickDuration, core,time), and
    $(REF FracSec, core,time).
    core.time is publically imported into stdd.datetime, it isn't necessary
    ...
    $(REF Date,stdd,_datetime,date) (if they want dates but don't care about
    time), $(REF DateTime,stdd,_datetime,date) (if they want dates and times
    but don't care about time zones), $(REF SysTime,stdd,_datetime,systime) (if
    ...
    $(REF Date,stdd,_datetime,date) and $(REF DateTime,stdd,_datetime,date) are
    optimized for calendar-based operations, while
    $(REF SysTime,stdd,_datetime,systime) is designed for dealing with time from
    the OS. Check out their specific documentation for more details.

    To get the current time, use $(REF Clock.currTime,stdd,_datetime,systime).
    It will return the current time as a $(REF SysTime,stdd,_datetime,systime). To
    ...
    create a $(REF SysTime,stdd,_datetime,systime) from the string.
...
--------------------
    ...
    nothing in stdd.datetime has precision greater than hnsecs, and very little
    in core.time does, no functions in stdd.datetime accept $(D "nsecs").
    ...
    Note:
        $(REF DateTimeException,stdd,_datetime,date) is an alias for
        $(REF TimeException, core,time), so you don't need to worry about
        core.time functions and stdd.datetime functions throwing different
        exception types (except in the rare case that they throw something other
        than $(REF TimeException, core,time) or
        $(REF DateTimeException,stdd,_datetime,date)).

    See_Also:
        $(DDLINK intro-to-_datetime, Introduction to stdd.datetime,
                 Introduction to stdd&#46;_datetime)<br>
    ...
    Source:    $(PHOBOSSRC stdd/_datetime/package.d)
+/
module stdd.datetime;

public import core.time;
public import stdd.datetime.date;
public import stdd.datetime.interval;
public import stdd.datetime.systime;
public import stdd.datetime.timezone;
...
@safe unittest
{
    assert(stdd.datetime.Date(2010, 9, 7) + dur!"days"(5) ==
           stdd.datetime.Date(2010, 9, 12));

    assert(stdd.datetime.Date(2010, 9, 7) - stdd.datetime.Date(2010, 10, 3) ==
           dur!"days"(-26));
}

@safe unittest
{
    import stdd.traits : hasUnsharedAliasing;
    /* Issue 6642 */
...
// @@@DEPRECATED_2018-10@@@
/++
    $(RED The old benchmarking functionality in stdd.datetime (which uses
          $(REF TickDuration,core,time)) has been deprecated. Use what's in
          stdd.datetime.stopwatch instead. It uses $(REF MonoTime,core,time) and
          $(REF Duration,core,time). See
          $(REF AutoStart,stdd,datetime,stopwatch). This symbol will be removed
    ...
    it is constructed.
  +/
deprecated("Use stdd.datetime.stopwatch.AutoStart.") alias AutoStart = Flag!"autoStart";
...
/++
    $(RED The old benchmarking functionality in stdd.datetime (which uses
          ...
          stdd.datetime.stopwatch instead. It uses $(REF MonoTime,core,time) and
          $(REF Duration,core,time). See
          $(REF _StopWatch,stdd,datetime,stopwatch). This symbol will be removed
          from the documentation in October 2018 and fully removed from Phobos
          in October 2019.)
  ...
  +/
deprecated("Use stdd.datetime.stopwatch.StopWatch.")
...
/++
    $(RED The old benchmarking functionality in stdd.datetime (which uses
          ...
          stdd.datetime.stopwatch instead. It uses $(REF MonoTime,core,time) and
          $(REF Duration,core,time). See
          $(REF benchmark,stdd,datetime,stopwatch). This symbol will be removed
          from the documentation in October 2018 and fully removed from Phobos
          in October 2019.)
    ...
    See_Also:
        $(LREF measureTime)
  +/
deprecated("Use stdd.datetime.stopwatch.benchmark.")
...
/++
    $(RED The old benchmarking functionality in stdd.datetime (which uses
          ...
          See $(REF benchmark,stdd,datetime,stopwatch). This symbol will be
          removed from the documentation in October 2018 and fully removed from
          Phobos in October 2019.)

    Return value of benchmark with two functions comparing.
  +/
deprecated("Use stdd.datetime.stopwatch.benchmark.") @safe struct ComparingBenchmarkResult
{
    /++
       Evaluation value
...
/++
    $(RED The old benchmarking functionality in stdd.datetime (which uses
          ...
          $(REF StopWatch,stdd,datetime,stopwatch). This symbol will be removed
          from the documentation in October 2018 and fully removed from Phobos
          in October 2019.)
...
--------------------
    ...
    See_Also:
        $(LREF benchmark)
+/
deprecated("Use stdd.datetime.stopwatch.StopWatch.") @safe auto measureTime(alias func)()
...
/// Ditto
deprecated("Use stdd.datetime.stopwatch.StopWatch.") auto measureTime(alias func)()
if (!isSafe!((){StopWatch sw; unaryFun!func(sw.peek());}))
{
    struct Result
    {
