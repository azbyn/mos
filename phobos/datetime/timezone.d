...
$(TR $(TD Utilities) $(TD
    ...
    Source:    $(PHOBOSSRC stdd/datetime/_timezone.d)
+/
module stdd.datetime.timezone;
...
import stdd.datetime.systime;// : Clock, stdTimeToUnixTime, SysTime;
import stdd.range.primitives;// : back, front, empty, popFront;
import stdd.traits : isIntegral, isSomeString, Unqual;
...
version(unittest) import stdd.exception : assertThrown;


/++
    Represents a time zone. It is used with $(REF SysTime,stdd,datetime,systime)
    to indicate the time zone of a $(REF SysTime,stdd,datetime,systime).
  +/
...
public:
    ...
    /++
        ...
        in UTC time (i.e. stdd time) and returns whether DST is effect in this
    ...
    /++
        Takes the number of hnsecs (100 ns) since midnight, January 1st, 1 A.D.
        in UTC time (i.e. stdd time) and converts it to this time zone's time.
    ...
    /++
        Takes the number of hnsecs (100 ns) since midnight, January 1st, 1 A.D.
        in this time zone's time and converts it to UTC (i.e. stdd time).
    ...
    /++
        Returns what the offset from UTC is at the given stdd time.
    ...
    static immutable(TimeZone) getTimeZone(string name) @safe
    {
        ...
        {
            import stdd.datetime.date : DateTimeException;
    ...
    @system unittest
    {
        ...
        import stdd.conv : to;
        import stdd.file : exists, isFile;
        import stdd.format : format;
                                          ...
                                          bool north = true)
        {
            ...
            version(Posix)
            {
                ...
                if (chainPath(PosixTimeZone.defaultTZDatabaseDir, "right", tzName).exists)
                {
                    ...
                    //assert(leapSTD.adjTime - leapDiff == stdd.adjTime);
        ...
        foreach (i; 0 .. tzs.length)
        {
            ...
            // in a SysTime with the correct stdd time during and surrounding
            // a DST switch.
            foreach (hour; -12 .. 13)
            {
...
protected:
    ...
    /++
        Params:
            ...
            stdName = The abbreviation for the time zone during stdd time.
...
public:
    ...
    /++
        Takes the number of hnsecs (100 ns) since midnight, January 1st, 1 A.D.
        in UTC time (i.e. stdd time) and returns whether DST is in effect in this
        time zone at the given point in time.
    ...
    /++
        ...
        systems to adjust the time to the appropriate time zone from stdd time.
    ...
    /++
        Returns stdd time using the standard C function calls on Posix systems
...
private:
    ...
    static immutable(LocalTime) singleton() @trusted
    {
        ...
        import stdd.concurrency : initOnce;
...
public:
    ...
    @safe unittest
    {
        ...
        version(Posix)
        {
            ...
            setTZEnvVar("UTC");
            import stdd.datetime.date : Date;
            auto stdd = SysTime(Date(2010, 1, 1));
            auto dst = SysTime(Date(2010, 7, 1));
            assert(UTC().utcToTZ(stdd.stdTime) == stdd.stdTime);
    ...
    @safe unittest
    {
        ...
        version(Posix)
        {
            ...
            auto stdd = SysTime(Date(2010, 1, 1));
            auto dst = SysTime(Date(2010, 7, 1));
            assert(UTC().tzToUTC(stdd.stdTime) == stdd.stdTime);
...
/++
    ...
    $(REF SysTime,stdd,datetime,systime)'s $(D fromISOString),
...
package:
    ...
    {
        import stdd.algorithm.searching : startsWith;
        import stdd.conv : text, to, ConvException;
        import stdd.datetime.date : DateTimeException;
        import stdd.exception : enforce;
...
{
    import stdd.algorithm.searching : countUntil, canFind, startsWith;
    import stdd.file : isDir, isFile, exists, dirEntries, SpanMode, DirEntry;
    import stdd.path : extension;
    import stdd.stdio : File;
    import stdd.string : strip, representation;
    import stdd.traits : isArray, isSomeChar;
public:
    ...
    /++
        ...
        Throws:
            $(REF DateTimeException,stdd,datetime,date) if the given time zone
    ...
    static immutable(PosixTimeZone) getTimeZone(string name, string tzDatabaseDir = defaultTZDatabaseDir) @trusted
    {
        ...
        import stdd.exception : enforce;
        import stdd.format : format;
        import stdd.path : asNormalizedPath, chainPath;
...
private:
    ...
    struct TransitionType
    {
        ...
        // Whether the transition is in stdd time (as opposed to wall clock time).
        bool isStd;
    ...
    {
        import stdd.bitmanip : bigEndianToNative;
        T[1] buff;
    ...
    /+
        Throws:
            $(REF DateTimeException,stdd,datetime,date) if $(D result) is false.
      +/
...
version(StdDdoc)
{
    ...
    public:
        ...
        /++
            ...
            1 A.D. in UTC time (i.e. stdd time) and returns whether DST is in
        ...
        /++
            Takes the number of hnsecs (100 ns) since midnight, January 1st,
            1 A.D. in UTC time (i.e. stdd time) and converts it to this time
                zone's time.
        ...
        /++
            Takes the number of hnsecs (100 ns) since midnight, January 1st,
            1 A.D. in this time zone's time and converts it to UTC (i.e. stdd
            time).
        ...
        /++
            ...
            Throws:
                $(REF DateTimeException,stdd,datetime,date) if the given time
    ...
    private:
        ...
        static bool _dstInEffect(const TIME_ZONE_INFORMATION* tzInfo, long stdTime) @trusted nothrow
        {
            try
            {
                ...
                import stdd.datetime.date : DateTime, Month;
                ...
                {
                    import stdd.datetime.date : Month;
...
--------------------
    ...
    auto text = stdd.file.readText("path/to/windowsZones.xml");
    ...
    auto conversions2 = parseTZConversions(stdd.net.curl.get(url));
--------------------
  +/
...
TZConversions parseTZConversions(string windowsZonesXMLText) @safe pure
{
    ...
    // needing to pull in stdd.xml (which we're theoretically replacing at some
    // point anyway).
    import stdd.algorithm.iteration : uniq;
    import stdd.algorithm.searching : find;
    import stdd.algorithm.sorting : sort;
    import stdd.array : array, split;
...
int main(string[] args)
{
    ...
    stdd.file.write(nix2WinFile, nix2WinLines.join("\n"));

    auto win2NixFile = args[3];
    stdd.file.write(win2NixFile, win2NixLines.join("\n"));
