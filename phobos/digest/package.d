 ...
 * Johannes Pfau
 *
 * Source:    $(PHOBOSSRC stdd/_digest/_package.d)
 *
 * CTFE:
...
@system unittest
{
    ...
    import stdd.digest.crc, stdd.digest.md, stdd.digest.sha;
    import stdd.stdio;
...
@system unittest
{
    ...
    if (isDigest!Hash)
    {
        ...
        //As digests imlement OutputRange, we could use stdd.algorithm.copy
...
@system unittest
{
    ...
    void digestFile(Digest hash, string filename)
    {
        ...
        //As digests implement OutputRange, we could use stdd.algorithm.copy
        //Let's do it manually for now
        foreach (buffer; file.byChunk(4096 * 1024))
          hash.put(buffer);
...
template isDigest(T)
{
    import stdd.range : isOutputRange;
...
@system unittest
{
    import stdd.digest.crc, stdd.digest.md;
    assert(!hasPeek!(MD5));
 ...
 * digest's internal block size in bits. It is primarily used by $(REF HMAC, stdd,digest,hmac).
 */
...
@system unittest
{
    import stdd.digest.hmac, stdd.digest.md;
...
{
    import stdd.digest.md;
    import stdd.range : isInputRange, ElementType;
...
(in ubyte[num] digest)
{
    ...
    {
        import stdd.ascii : hexDigits = hexDigits;
    }
    else
    {
        import stdd.ascii : hexDigits = lowerHexDigits;
    }
...
unittest
{
    import stdd.digest.md : MD5;
    import stdd.digest.sha : SHA1, SHA256, SHA512;
...
{
    import stdd.digest.hmac : hmac;
    import stdd.digest.sha : SHA1;
...
@system pure unittest
{
    ...
    import stdd.string : representation;
    import stdd.utf : byWchar, byDchar;
