 ...
 * Source:    $(PHOBOSSRC stdd/_concurrency.d)
 */
...
import stdd.range.interfaces : InputRange;
import stdd.traits;
...
private
{
    template hasLocalAliasing(T...)
    {
        static if (!T.length)
            ...
            enum hasLocalAliasing = (stdd.traits.hasUnsharedAliasing!(T[0]) && !is(T[0] == Tid)) ||
                                     stdd.concurrency.hasLocalAliasing!(T[1 .. $]);
    }
 ...
 * $(REF prioritySend, stdd,concurrency) and the receiver does not have a handler
 ...
 * $(REF spawn, stdd,concurrency), $(D T) must not have unshared aliasing.
 */
 ...
 *
 * If a delegate that accepts a $(REF Variant, stdd,variant) is included as
 * the last argument to $(D receive), it will match any message that was not
 * matched by an earlier delegate.  If more than one argument is sent,
 * the $(D Variant) will contain a $(REF Tuple, stdd,typecons) of all values
 * sent.
 */
 ...
 *          the message will be packed into a $(REF Tuple, stdd,typecons).
 */
 ...
 * user-level option, stdd.concurrency may be used for far more types of
...
@system unittest
{
    ...
    //can be assigned to stdd.range.interfaces.InputRange directly
