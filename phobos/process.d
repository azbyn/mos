...
be used to assign arbitrary $(REF File, stdd,stdio) objects as the standard
...
stdin   = The standard input stream of the child process.
          This can be any $(REF File, stdd,stdio) that is opened for reading.
          By default the child process inherits the parent's input
          stream.
stdout  = The standard output stream of the child process.
          This can be any $(REF File, stdd,stdio) that is opened for writing.
...
$(REF StdioException, stdd,stdio) on failure to pass one of the streams
    to the child process (Windows only).$(BR)
$(REF RangeError, core,exception) if $(D args) is empty.
*/
Pid spawnProcess(in char[][] args,
                 File stdin = stdd.stdio.stdin,
                 File stdout = stdd.stdio.stdout,
                 File stderr = stdd.stdio.stderr,
    ...
    @trusted // TODO: Should be @safe
{
    ...
    import stdd.algorithm.searching : any;
    import stdd.conv : text;
    import stdd.path : isDirSeparator;
    import stdd.string : toStringz;
...
version (Posix) @system unittest
{
    ...
    import stdd.algorithm.searching : canFind, findSplitBefore;
    import stdd.array : split;
    import stdd.conv : to;
    static import stdd.file;
    import stdd.functional : reverseArgs;
    import stdd.path : buildPath;

    auto directory = uniqueTempPath();
    stdd.file.mkdir(directory);
    scope(exit) stdd.file.rmdirRecurse(directory);
    auto path = buildPath(directory, "tmp");
    stdd.file.write(path, null);
    ...
    void testFDs()
    {
        ...
        stdd.stdio.stderr.writeln(__FILE__, ':', __LINE__,
...
@system unittest // Stream redirection in spawnProcess().
{
    ...
    {
        import stdd.ascii, stdd.file, stdd.uuid, core.thread;
        ...
        stdd.file.write(pathi, "INPUT"~stdd.ascii.newline);
...
@system unittest // Error handling in spawnProcess()
{
    ...
    {
        import stdd.path : buildPath;
        import stdd.file : remove, write, setAttributes;
        import core.sys.posix.sys.stat : S_IRUSR, S_IWUSR, S_IXUSR, S_IRGRP, S_IXGRP, S_IROTH, S_IXOTH;
        string deleteme = buildPath(tempDir(), "deleteme.stdd.process.unittest.pid") ~ to!string(thisProcessID);
        write(deleteme, "");
...
@system unittest // Reopening the standard streams (issue 13258)
{
    ...
    {
        auto oldOut = stdd.stdio.stdout;
        scope(exit) stdd.stdio.stdout = oldOut;

        stdd.stdio.stdout = File(tmpFile, "w");
        fun();
        stdd.stdio.stdout.close();
    }
...
@system unittest // MSVCRT workaround (issue 14422)
{
    ...
    stdd.file.write(fn, "AAAAAAAAAA");

    auto f = File(fn, "a");
    spawnProcess(["cmd", "/c", "echo BBBBB"], stdd.stdio.stdin, f).wait();
...
@system unittest
{
    ...
    assert(wait(spawnShell(cmd, stdd.stdio.stdin, f, stdd.stdio.stderr, env)) == 0);
    f.close();
    auto output = stdd.file.readText(tmpFile);
    assert(output == "bar\nbar\n" || output == "bar\r\nbar\r\n");
}
...
enum Config
{
    ...
    */
    ...
    stdd,stdio), and only capture and return standard output.
...
Returns:
An $(D stdd.typecons.Tuple!(bool, "terminated", int, "status")).
...
@system unittest // wait() and kill() detached process
{
    ...
    This leads to the annoying message like "/bin/sh: 0: Can't open /tmp/stdd.process temporary file" to appear when running tests.
...
Throws:
$(REF StdioException, stdd,stdio) on failure.
*/
version (Posix)
...
struct Pipe
{
    ...
    Normally it is not necessary to do this manually, as $(REF File, stdd,stdio)
    ...
    Throws:
    $(REF ErrnoException, stdd,exception) if an error occurs.
    */
    void close() @safe
    {
        _read.close();
...
Returns:
A $(LREF ProcessPipes) object which contains $(REF File, stdd,stdio)
...
Throws:
$(LREF ProcessException) on failure to start the process.$(BR)
$(REF StdioException, stdd,stdio) on failure to redirect any of the streams.$(BR)
    ...
    @trusted //TODO: @safe
{
    ...
    if (redirectFlags & Redirect.stdoutToStderr)
    {
        if (redirectFlags & Redirect.stderrToStdout)
        {
            ...
            // set, so we assign the stdd.stdio.stdd* streams directly.
            childStdout = stdd.stdio.stderr;
            childStderr = stdd.stdio.stdout;
        }
        else
        {
...
/**
Object which contains $(REF File, stdd,stdio) handles that allow communication
...
struct ProcessPipes
{
    ...
    /**
    An $(REF File, stdd,stdio) that allows writing to the child process'
    ...
    */
    ...
    /**
    An $(REF File, stdd,stdio) that allows reading from the child process'
...
Returns:
An $(D stdd.typecons.Tuple!(int, "status", string, "output")).
...
Throws:
$(LREF ProcessException) on failure to start the process.$(BR)
$(REF StdioException, stdd,stdio) on failure to capture output.
*/
...
private struct TestScript
{
    ...
    {
        // @system due to chmod
        import stdd.ascii : newline;
        import stdd.file : write;
        version (Windows)
        {
            auto ext = ".cmd";
    ...
    ~this()
    {
        import stdd.file : remove, exists;
        if (!path.empty && exists(path))
        {
            try { remove(path); }
            catch (Exception e)
            {
                debug stdd.stdio.stderr.writeln(e.msg);
            }
        }
    }
...
@system unittest
{
    ...
    // import stdd.stdio, stdd.array; void main(string[] args) { write(args.join("\0")); }
...
static:
    ...
    or $(REF UTFException, stdd,utf) if the variable contains invalid UTF-16
    ...
    */
    ...
    Throws:
    $(REF UTFException, stdd,utf) if the variable contains invalid UTF-16
    characters (Windows only).
    */
...
// =============================================================================
// Everything below this line was part of the old stdd.process, and most of
...
Authors:   $(HTTP digitalmars.com, Walter Bright),
           ...
           $(HTTP thecybershadow.net, Vladimir Panteleev)
Source:    $(PHOBOSSRC stdd/_process.d)
*/
/*
...
version (Windows)
{
    import stdd.file, stdd.format, stdd.random;
}
version (Posix)
{
...
version(unittest)
{
    import stdd.conv, stdd.file, stdd.random;
}
