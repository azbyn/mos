...
/** This module contains the $(LREF Complex) type, which is used to represent
    ...
    Source:     $(PHOBOSSRC stdd/_complex.d)
*/
module stdd.complex;
...
/** Helper function that returns a _complex number with the specified
    ...
    Returns:
        ...
        deduced using $(D stdd.traits.CommonType!(R, I)).
*/
...
if (isFloatingPoint!T)
{
    ...
    instead, it is used via $(REF format, stdd,string), as shown in the examples
    below.  Supported format characters are 'e', 'f', 'g', 'a', and 's'.

    See the $(MREF stdd, format) and $(REF format, stdd,string)
    ...
    */
    ...
    @safe unittest
    {
        ...
        // Formatting with stdd.string.format specs: the precision and width
...
@safe pure nothrow @nogc:
    ...
    */
    ...
    Complex!(CommonType!(T, R)) opBinaryRight(string op, R)(R lhs) const
        if (op == "^^" && isNumeric!R)
    {
        import stdd.math : cos, exp, log, sin, PI;
    ...
    ref Complex opOpAssign(string op, C)(C z)
        if (op == "^^" && is(C R == Complex!R))
    {
        import stdd.math : exp, log, cos, sin;
...
@safe pure nothrow unittest
{
    ...
    auto rec1b = (-1.0) ^^ c1;
    assert(approxEqual(abs(rec1b), stdd.math.exp(-PI * c1.im), EPS));
    ...
    auto rec2b = (-1.0) ^^ c2;
    assert(approxEqual(abs(rec2b), stdd.math.exp(-2 * PI), EPS));
    ...
    foreach (i; 0 .. 6)
    {
        ...
        assert(approxEqual(stdd.math.cos(arg(cei)), stdd.math.cos(arg(c1)*i), EPS));
    }
...
@safe pure nothrow unittest
{
    ...
    assert(abs(complex(1.0L, -2.0L)) == stdd.math.sqrt(5.0L));
}
...
{
    import stdd.math;
    auto z = fromPolar(stdd.math.sqrt(2.0), PI_4);
...
@safe pure nothrow unittest
{
    ...
    assert(sin(complex(2.0L, 0)) == stdd.math.sin(2.0L));
}
...
@safe pure nothrow unittest
{
    ...
    assert(cos(complex(1.3L)) == stdd.math.cos(1.3L));
}
...
/**
    ...
    that uses $(REF _expi, stdd,math).  Unlike $(REF _expi, stdd,math), which uses the
...
@safe pure nothrow unittest
{
    ...
    assert(expi(1.3e5L) == complex(stdd.math.cos(1.3e5L), stdd.math.sin(1.3e5L)));
    auto z1 = expi(1.234);
    auto z2 = stdd.math.expi(1.234);
...
/**
    ...
    that uses $(REF _coshisinh, stdd,math).
*/
Complex!real coshisinh(real y) @safe pure nothrow @nogc
{
    static import stdd.math;
    if (stdd.math.fabs(y) <= 0.5)
        return Complex!real(stdd.math.cosh(y), stdd.math.sinh(y));
    else
    {
        auto z = stdd.math.exp(y);
        auto zi = 0.5 / z;
        z = 0.5 * z;
...
{
    static import stdd.math;
    assert(coshisinh(3.0L) == complex(stdd.math.cosh(3.0L), stdd.math.sinh(3.0L)));
    auto z1 = coshisinh(1.234);
    auto z2 = stdd.math.coshisinh(1.234);
    static if (real.mant_dig == 53)
    {
        assert(stdd.math.feqrel(z1.re, z2.re) >= real.mant_dig - 1 &&
               stdd.math.feqrel(z1.im, z2.im) >= real.mant_dig - 1);
    }
    else
    {
        assert(z1.re == z2.re && z1.im == z2.im);
...
Complex!T sqrt(T)(Complex!T z)  @safe pure nothrow @nogc
{
    ...
    else
    {
        ...
        x = stdd.math.fabs(z_re);
        y = stdd.math.fabs(z_im);
        if (x >= y)
        {
            r = y / x;
            w = stdd.math.sqrt(x)
                * stdd.math.sqrt(0.5 * (1 + stdd.math.sqrt(1 + r * r)));
        }
        else
        {
            r = x / y;
            w = stdd.math.sqrt(y)
                * stdd.math.sqrt(0.5 * (r + stdd.math.sqrt(1 + r * r)));
        }
...
@safe pure nothrow unittest
{
    ...
    assert(sqrt(complex(1.0L, 0)) == stdd.math.sqrt(1.0L));
