 ...
 * Source:    $(PHOBOSSRC stdd/_base64.d)
 * Macros:
...
import stdd.range.primitives;      // isInputRange, isOutputRange, isForwardRange, ElementType, hasLength
...
template Base64Impl(char Map62th, char Map63th, char Padding = '=')
{
     ...
     *
     * Params:
     *  source = The $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
     *           to _encode.
     ...
     * $(REF_ALTTEXT output range, isOutputRange, stdd,range,primitives) using
     * Base64 encoding.
     *
     * Params:
     *  source = The $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
     *           to _encode.
     *  range  = The $(REF_ALTTEXT output range, isOutputRange, stdd,range,primitives)
     ...
     *
     * This range will be a $(REF_ALTTEXT forward range, isForwardRange, stdd,range,primitives)
     ...
     *
     * It will be a $(REF_ALTTEXT forward range, isForwardRange, stdd,range,primitives)
     * if the underlying data source is at least a forward range.
     *
     * Note: This struct is not intended to be created in user code directly;
     * use the $(LREF encoder) function instead.
     ...
     * given $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives).
     *
     * Params:
     ...
     *  $(REF_ALTTEXT forward range, isForwardRange, stdd,range,primitives) if the
...
@system unittest
{
    ...
    { // Encoder and Decoder
        {
            string encode_file = stdd.file.deleteme ~ "-testingEncoder";
            stdd.file.write(encode_file, "\nf\nfo\nfoo\nfoob\nfooba\nfoobar");
        ...
        {
            string decode_file = stdd.file.deleteme ~ "-testingDecoder";
            stdd.file.write(decode_file, "\nZg==\nZm8=\nZm9v\nZm9vYg==\nZm9vYmE=\nZm9vYmFy");
