...
import stdd.datetime.systime : Clock, SysTime, unixTimeToStdTime;
...
version (Posix) private void[] readImpl(const(char)[] name, const(FSChar)* namez, size_t upTo = size_t.max) @trusted
{
    ...
    import stdd.conv : to;
    import stdd.experimental.checkedint : checked;
...
/++
    Reads and validates (using $(REF validate, stdd, utf)) a text file. S can be
    ...
    Throws: $(LREF FileException) if there is an error reading the file,
            $(REF UTFException, stdd, utf) on UTF decoding error.
+/
...
if (isSomeString!S && (isInputRange!R && !isInfinite!R && isSomeChar!(ElementType!R) || is(StringTypeOf!R)))
{
    ...
    import stdd.encoding : getBOM, BOM;
    import stdd.exception : enforce;
    import stdd.format : format;
    import stdd.utf : UTFException, validate;
...
@system unittest
{
    ...
    import stdd.bitmanip : append, Endian;
    import stdd.exception : assertThrown;
    import stdd.path : buildPath;
    ...
    !isConvertibleToString!R)
{
    version(Windows)
    {
        import stdd.datetime.systime : FILETIMEToSysTime;
    ...
    !isConvertibleToString!R)
{
    version(Windows)
    {
        import stdd.datetime.systime : SysTimeToFILETIME;
...
@system unittest
{
    ...
    import stdd.array : array;
    import stdd.conv : to;
    import stdd.path : buildPath, absolutePath;
    import stdd.file : dirEntries;
    import stdd.process : thisProcessID;
 ...
 * passed to $(REF formattedRead, stdd,_format), and therefore must conform to the
 * _format string specification outlined in $(MREF stdd, _format).
 *
 * Params:
 ...
 *     to $(REF formattedRead, stdd,_format). Meaning that no empty lines or lines
