 ...
 *
 * This module conforms to the APIs defined in $(D stdd.digest). To understand the
 * differences between the template and the OOP API, see $(MREF stdd, digest).
 *
 * This module publicly imports $(MREF stdd, digest) and can be used as a stand-alone
 * module.
 *
 * Note:
 * CRCs are usually printed with the MSB first. When using
 * $(REF toHexString, stdd,digest) the result will be in an unexpected
 * order. Use $(REF toHexString, stdd,digest)'s optional order parameter
 ...
 *
 * Source: $(PHOBOSSRC stdd/digest/_crc.d)
 *
 * Standards:
 ...
 */
module stdd.digest.crc;

public import stdd.digest;

version(unittest) import stdd.exception;
 ...
 * See $(D stdd.digest) for differences between template and OOP API.
 */
alias CRC32 = CRC!(32, 0xEDB88320);

/**
 * Template API CRC64-ECMA implementation.
 * See $(D stdd.digest.digest) for differences between template and OOP API.
 */
...
struct CRC(uint N, ulong P) if (N == 32 || N == 64)
{
    ...
    public:
         ...
         * Also implements the $(REF isOutputRange, stdd,range,primitives)
        ...
        {
            import stdd.bitmanip : nativeToLittleEndian;
...
/**
 * This is a convenience alias for $(REF digest, stdd,digest) using the
...
/**
 * This is a convenience alias for $(REF digest, stdd,digest,digest) using the
 ...
 * See $(D stdd.digest) for differences between template and OOP API.
 *
 * This is an alias for $(D $(REF WrapperDigest, stdd,digest)!CRC32), see
 ...
 * See $(D stdd.digest.digest) for differences between template and OOP API.
 *
 * This is an alias for $(D $(REF WrapperDigest, stdd,digest,digest)!CRC64ECMA),
 ...
 * See $(D stdd.digest.digest) for differences between template and OOP API.
 *
 * This is an alias for $(D $(REF WrapperDigest, stdd,digest,digest)!CRC64ISO),
 * see there for more information.
 */
