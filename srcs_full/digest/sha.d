 ...
 *
 * This module conforms to the APIs defined in $(MREF stdd, digest). To understand the
 * differences between the template and the OOP API, see $(MREF stdd, digest).
 *
 * This module publicly imports $(D stdd.digest) and can be used as a stand-alone
 * module.
 *
...
/**
 * These are convenience aliases for $(REF digest, stdd,digest) using the
 * SHA implementation.
 */
 ...
 * See $(D stdd.digest) for differences between template and OOP API.
 *
 * This is an alias for $(D $(REF WrapperDigest, stdd,digest)!SHA1), see
