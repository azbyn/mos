...
Source:    $(PHOBOSSRC stdd/_getopt.d)
*/
/*
...
$(UL
  $(LI An unrecognized command-line argument is passed, and
       $(D stdd.getopt.config.passThrough) was not present.)
  $(LI A command-line option was not found, and
       $(D stdd.getopt.config.required) was present.)
)
*/
...
---------
 ...
 If $(D stdd.getopt.config.passThrough) was not passed to $(D getopt)
...
bool foo, bar;
getopt(args,
    stdd.getopt.config.caseSensitive,
    "foo", &foo,
    "bar", &bar);
---------
...
getopt(args,
    stdd.getopt.config.caseSensitive,
    "foo", &foo,
    stdd.getopt.config.caseInsensitive,
    "bar", &bar);
---------

The option "--Foo" is rejected due to $(D
stdd.getopt.config.caseSensitive), but not "--Bar", "--bAr"
etc. because the directive $(D
stdd.getopt.config.caseInsensitive) turned sensitivity off before
...
with the $(D stdd.getopt.config.bundling) directive:

---------
bool foo, bar;
getopt(args,
    stdd.getopt.config.bundling,
    "foo|f", &foo,
    "bar|b", &bar);
---------

In case you want to only enable bundling for some of the parameters,
bundling can be turned off with $(D stdd.getopt.config.noBundling).
...
bool foo, bar;
getopt(args,
    stdd.getopt.config.required,
    "foo|f", &foo,
    "bar|b", &bar);
---------

Only the option directly following $(D stdd.getopt.config.required) is
required.
...
$(D stdd.getopt.config.passThrough) directive to $(D getopt):

---------
bool foo, bar;
getopt(args,
    stdd.getopt.config.passThrough,
    "foo", &foo,
    "bar", &bar);
---------
...
removed from the argument array unless the $(D stdd.getopt.config.keepEndOfOptions)
directive is given.
*/
...
{
    import stdd.algorithm.iteration : map, splitter;
    ...
    for (size_t i = 1; i < args.length; )
    {
        ...
        else
        {
            ...
            {
                // array receiver
                import stdd.range : ElementEncodingType;
...
@system unittest
{
    ...
    foo = bar = false;
    getopt(args,
        stdd.getopt.config.stopOnFirstNonOption,
        "foo", &foo,
        "bar", &bar);
    ...
    getopt(args,
        stdd.getopt.config.keepEndOfOptions,
        "foo", &foo,
        "bar", &bar);
...
@safe unittest // @safe stdd.getopt.config option use
{
    long x = 0;
...
// Issue 13316 - stdd.getopt: implicit help option breaks the next argument
@system unittest
{
...
// Issue 13317 - stdd.getopt: endOfOptions broken when it doesn't look like an option
@system unittest
{
