 ...
 */
template Mostdderived(T, TList...)
{
    static if (TList.length == 0)
        alias Mostdderived = T;
    else static if (is(TList[0] : T))
        alias Mostdderived = Mostdderived!(TList[0], TList[1 .. $]);
    else
        alias Mostdderived = Mostdderived!(T, TList[1 .. $]);
}
...
@safe unittest
{
    ...
    Mostdderived!(Object, Types) x;  // x is declared as type C
...
template DerivedToFront(TList...)
{
    static if (TList.length == 0)
        ...
        alias DerivedToFront =
            AliasSeq!(Mostdderived!(TList[0], TList[1 .. $]),
                       DerivedToFront!(ReplaceAll!(Mostdderived!(TList[0], TList[1 .. $]),
                                TList[0],
                                TList[1 .. $])));
}
...
/**
 * Converts an $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
