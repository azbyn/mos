...
Source:    $(PHOBOSSRC stdd/_typecons.d)
Authors:   $(HTTP erdani.org, Andrei Alexandrescu),
           ...
           Kenji Hara
 */
module stdd.typecons;

import core.stdc.stdint : uintptr_t;
import stdd.format : singleSpec, FormatSpec, formatValue;
...
public:
    ...
    */
    ...
    Same as calling stdd.algorithm.move on it.
    */
    Unique release()
    {
...
loop constructs (e.g. $(REF AliasSeq, stdd,meta) iteration), all of which use
zero-based indexing.
    ...
    struct Tuple
    {
        ...
        if (names.length == 0 || allSatisfy!(isSomeString, typeof(names)))
        {
            ...
            else
            {
                ...
                import stdd.range : roundRobin, iota;
        ...
        static if (Specs.length == 0) @safe unittest
        {
            ...
            import stdd.range : iota, zip;
            import stdd.algorithm.iteration : map, sum;
        ...
        */
        ...
        template toString()
        {
            ...
            {
                import stdd.format : formatElement, formattedWrite, FormatException;
                if (fmt.nested)
                {
                    if (fmt.sep)
                    {
...
/**
    Returns `true` if and only if `T` is an instance of `stdd.typecons.Tuple`.
 ...
 */
     ...
     * result is equivalent to calling $(REF formattedWrite, stdd,format) on the
     * underlying value.
     *
     * Params:
     *     writer = A `char` accepting
     *     $(REF_ALTTEXT output range, isOutputRange, stdd, range, primitives)
     *     fmt = A $(REF FormatSpec, stdd,format) which is used to represent
    ...
    {
        import stdd.range.primitives : put;
        if (isNull)
...
string generateLogger(C, alias fun)() @property
{
    ...
    stmt ~= q{ struct Importer { import stdd.stddio; } };
...
Generated code is inserted in the scope of `stdd.typecons` module.  Thus,
any useful functions outside `stdd.typecons` cannot be used in the generated
...
$(UL
 ...
 $(LI Deep interface inheritance causes compile error with messages like
      "Error: function stdd.typecons._AutoImplement!(Foo)._AutoImplement.bar
...
if (is(T == class) || is(T == interface))
{
    ...
    if (is(S == class) || is(S == interface))
    {
        ...
        {
            import stdd.traits : QualifierOf;
...
if (Targets.length >= 1 && allSatisfy!(isMutable, Targets))
{
    ...
    if (!allSatisfy!(Bind!(isImplicitlyConvertible, Source), Targets))
    {
        ...
        private:
            ...
            import stdd.conv : to;
            import stdd.functional : forward;
...
package template GetOverloadedMethods(T)
{
    ...
    template follows(size_t i = 0)
    {
        ...
        else
        {
            ...
            alias follows = AliasSeq!(
                stdd.meta.Filter!(isMethod, __traits(getOverloads, T, name)),
                follows!(i + 1));
        }
    }
...
if (!is(T == class) && !(is(T == interface)))
{
    ...
    struct RefCountedStore
    {
        ...
        {
            import core.exception : onOutOfMemoryError;
            import core.stddc.string : memcpy, memset;
            ...
            // Can't use stdd.algorithm.move(source, _store._payload)
...
if (is(T == class))
{
    ...
    static struct Scoped
    {
        ...
        @property inout(T) Scoped_payload() inout
        {
            ...
            if (d != *currD)
            {
                import core.stddc.string : memmove;
...
placeholder type `This` in $(REF Algebraic, stdd,variant).
...
{
    alias Constddg = void delegate(float) const;
    alias B = void delegate(int) const;
    alias A = ReplaceType!(float, int, Constddg);
