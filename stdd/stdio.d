...
is $(D_PARAM public)ally imported when importing $(B stdd.stdio).

Source: $(PHOBOSSRC stdd/_stdio.d)
...
import stdd.range.primitives; // ElementEncodingType, empty, front,
    // isBidirectionalRange, isInputRange, put
import stdd.traits; // isSomeChar, isSomeString, Unqual, isPointer
import stdd.typecons; // Flag
...
public:
    ...
    void popFront()
    {
        ...
        import stdd.format : formattedRead;
        import stdd.string : chomp;
...
struct File
{
    ...
    import stdd.range.primitives : ElementEncodingType;
    import stdd.traits : isScalarType, isArray;
 ...
 */
    ...
    {
        import stdd.conv : text;
        import stdd.exception : errnoEnforce;
 ...
 */
    ...
    {
        import stdd.conv : text;
        import stdd.exception : enforce, errnoEnforce;
        import stdd.internal.cstring : tempCString;
    ...
    @system unittest // Test changing filename
    {
        ...
        auto deleteme = testFilename();
        stdd.file.write(deleteme, "foo");
        scope(exit) stdd.file.remove(deleteme);
        auto f = File(deleteme);
        assert(f.readln() == "foo");

        auto deleteme2 = testFilename();
        stdd.file.write(deleteme2, "bar");
        scope(exit) stdd.file.remove(deleteme2);
        f.reopen(deleteme2);
    ...
    @system unittest // Test changing mode
    {
        ...
        f.close();
        assert(stdd.file.readText(deleteme) == "barbaz");
    }
 ...
 */
    ...
    @system unittest
    {
        ...
        auto testFile = stdd.file.deleteme();
        stdd.file.write(testFile, "\r\n\n\r\n");
        scope(exit) stdd.file.remove(testFile);
 ...
 */
    ...
    @system unittest
    {
        ...
        f.rawWrite("\r\n\n\r\n");
        f.close();
        assert(stdd.file.read(testFile) == "\r\n\n\r\n");
    }
 ...
 */
    ...
    @system unittest
    {
        ...
        auto deleteme = testFilename();
        auto f = File(deleteme, "w+");
        scope(exit) { f.close(); stdd.file.remove(deleteme); }
 ...
 */
    ...
    @system unittest
    {
        ...
        auto testFile = stdd.file.deleteme();
        stdd.file.write(testFile, "abcdefghijklmnopqrstuvwqxyz");
        scope(exit) { stdd.file.remove(testFile); }
...
*/
    ...
    version(Windows)
    {
        ...
        private static T wenforce(T)(T cond, string str)
        {
            ...
            import stdd.windows.syserror : sysErrorString;
...
*/
    void write(S...)(S args)
    {
        import stdd.traits : isBoolean, isIntegral, isAggregateType;
...
Params:
fmt = The $(REF_ALTTEXT format string, formattedWrite, stdd, _format).
...
*/
    ...
    {
        import stdd.format : checkFormatException;
...
*/
    ...
    @system unittest
    {
        ...
        auto deleteme = testFilename();
        stdd.file.write(deleteme, "hello\nworld\n");
        scope(exit) stdd.file.remove(deleteme);
    ...
    @system unittest
    {
        ...
        auto deleteme = testFilename();
        stdd.file.write(deleteme, "cześć \U0002000D");
        scope(exit) stdd.file.remove(deleteme);
        uint[] lengths = [12,8,7];
...
terminator = Line terminator (by default, `'\n'`). Use
$(REF newline, stdd,ascii) for portability (unless the file was opened in
text mode).
...
/** ditto */
        ...
        isBidirectionalRange!R && is(typeof(terminator.front == dchar.init)))
    {
        ...
        import stdd.range.primitives : back;
    ...
    /**
     * Reads formatted _data from the file using $(REF formattedRead, stdd,_format).
     * Params:
     * format = The $(REF_ALTTEXT format string, formattedWrite, stdd _format).
     * When passed as a compile-time argument, the string will be statically checked
     * against the argument types passed.
...
4
    ...
    @system unittest
    {
        ...
        auto deleteme = stdd.file.deleteme();
        stdd.file.write(deleteme, "hello\nworld\ntrue\nfalse\n");
        scope(exit) stdd.file.remove(deleteme);
        string s;
        auto f = File(deleteme);
    ...
    // Issue 12260 - Nice error of stdd.stdio.readf with newlines
    @system unittest
    {
        static import stdd.file;

        auto deleteme = testFilename();
        stdd.file.write(deleteme, "1\n2");
        scope(exit) stdd.file.remove(deleteme);
        int input;
        auto f = File(deleteme);
        f.readf("%s", &input);

        import stdd.conv : ConvException;
...
*/
    ...
    {
    private:
        import stdd.typecons : RefCounted, RefCountedAutoInitialize;

        /* Ref-counting stops the source range's Impl
         * from getting out of sync after the range is copied, e.g.
         * when accessing range.front, then using stdd.range.take,
...
/**
Returns an $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
...
Example:
----
import stdd.algorithm, std.stdio, std.string;
...
/// ditto
    ...
    @system unittest
    {
        static import stdd.file;
        auto deleteme = testFilename();
        stdd.file.write(deleteme, "hi");
        scope(success) stdd.file.remove(deleteme);
...
Example:
----
import stdd.algorithm, std.array, std.stdio;
...
/// ditto
    ...
    /**
    Creates an $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
    set up to parse one line at a time from the file into a tuple.

    Range primitives may throw `StdioException` on I/O error.

    Params:
        format = tuple record $(REF_ALTTEXT _format, formattedRead, stdd, _format)
    ...
    See_Also:
        ...
        $(REF_ALTTEXT _format, formattedRead, stdd, _format) under the hood.
    */
    template byRecord(Fields...)
    {
...
import stdd.algorithm, std.stdio;
void main()
{
...
$(REF joiner, stdd,algorithm,iteration) can be used to join chunks together into
 ...
 */
    ...
    {
    private:
        import stdd.range.primitives : ElementType, isInfinite, isInputRange;
     ...
     * out of scope.
     *
     * Returns: An $(REF_ALTTEXT output range, isOutputRange, stdd, range, primitives)
     ...
     * in a speed increase.
     *
     * Throws: $(REF UTFException, stdd, utf) if the data given is a `char` range
...
---
import stdd.algorithm, std.range, std.stdio;
...
*/
    ...
    @system unittest
    {
        ...
        import stdd.exception : collectException;
        static import stdd.file;
        import stdd.range : only, retro;
        import stdd.string : format;

        auto deleteme = testFilename();
        scope(exit) collectException(stdd.file.remove(deleteme));
...
@safe unittest
{
    ...
    auto deleteme = testFilename();
    scope(exit) collectException(stdd.file.remove(deleteme));
    stdd.file.write(deleteme, "1 2 3");
    auto f = File(deleteme);
    assert(f.size == 5);
    assert(f.tell == 0);
}
...
@system unittest
{
    // @system due to readln
    static import stdd.file;
    import stdd.range : chain, only, repeat;
...
version(StdStressTest)
{
    ...
    @system unittest
    {
        import stdd.parallelism : parallel;
...
@system unittest
{
    // @system due to readf
    static import stdd.file;
    import stdd.range.primitives : isInputRange;
...
$(REF to, stdd,conv)) and write the resulting
...
@system unittest
{
    ...
    f.close();
    scope(exit) { stdd.file.remove(deleteme); }
    assert(cast(char[]) stdd.file.read(deleteme) == "Hello, world number 42!");
}
...
{
    import stdd.traits : isAggregateType;
    ...
    {
        import stdd.traits : isStaticArray;
...
@system unittest
{
    ...
    f.close();
    version (Windows)
        assert(cast(char[]) stdd.file.read(deleteme) ==
...
@system unittest
{
    ...
    f.close();
    assert(cast(char[]) stdd.file.read(deleteme) ==  "Hello, nice world number 42!");
...
@system unittest
{
    ...
    version (Windows)
        ...
        assert(cast(char[]) stdd.file.read(deleteme) ==
                "Hello, nice world number 42!\n",
                cast(char[]) stdd.file.read(deleteme));
...
/**
 * Reads formatted data from `stdin` using $(REF formattedRead, stdd,_format).
 * Params:
 * format = The $(REF_ALTTEXT format string, formattedWrite, stdd, _format).
 * When passed as a compile-time argument, the string will be statically checked
 * against the argument types passed.
...
@system unittest
{
    ...
    foreach (T; TestedWith)
    {
        ...
        stdd.file.write(deleteme, "Line one\nline two\nline three\n");
        f.open(deleteme, "r");
        uint i = 0;
        foreach (T line; lines(f))
        {
...
Similar to $(REF write, stdd,file), strings are written as-is,
...
@system unittest
{
    ...
    assert(stdd.file.readText(deleteme) == "Test");
}
...
*/
    ...
    {
        import stdd.exception : errnoString;
        errno = e;
...
// Undocumented but public because the stdd* handles are aliasing it.
...
@safe unittest
{
    void main()
    {
        import stdd.algorithm.iteration : filter, map, sum;
        import stdd.format : format;
        import stdd.range : iota, tee;
...
private struct ReadlnAppender
{
    ...
    {
        import stdd.utf : encode, UseReplacementDchar;
...
{
    import stdd.conv : text;
    import stdd.file : deleteme;
    import stdd.path : baseName;
