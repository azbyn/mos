...
import stdd.traits;     // isSomeChar, isSomeString
import stdd.typecons;   // Flag, Yes, No


/++
    Exception thrown on errors in stdd.utf functions.
  +/
class UTFException : Exception
{
    ...
    // FIXME: Use stdd.exception.basicExceptionCtors here once bug #11500 is fixed
...
/++
    ...
    Params:
        str = $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
...
version(unittest) private void testddecode(R)(R range,
                                             size_t index,
                                             dchar expectedChar,
...
version(unittest) private void testddecodeFront(R)(ref R range,
                                                  dchar expectedChar,
...
version(unittest) private void testddecodeBack(R)(ref R range,
                                                 dchar expectedChar,
                                                 size_t expectedNumCodeUnits,
                                                 size_t line = __LINE__)
{
...
version(unittest) private void testAllDecode(R)(R range,
                                                ...
                                                size_t line = __LINE__)
{
    testddecode(range, 0, expectedChar, expectedIndex, line);
    static if (isBidirectionalRange!R)
    {
        auto rangeCopy = range.save;
        testddecodeBack(rangeCopy, expectedChar, expectedIndex, line);
    }
    testddecodeFront(range, expectedChar, expectedIndex, line);
}
...
@system unittest
{
                          ...
                          (string s) => new RefRandomCU!char(s)))
    {
        ...
        {
            auto range = S("abcd");
            testddecode(range, 0, 'a', 1);
            testddecode(range, 1, 'b', 2);
            testddecodeFront(range, 'a', 1);
            testddecodeFront(range, 'b', 1);
        ...
        {
            auto range = S("ウェブサイト");
            testddecode(range, 0, 'ウ', 3);
            testddecode(range, 3, 'ェ', 6);
            testddecodeFront(range, 'ウ', 3);
            testddecodeFront(range, 'ェ', 3);
        ...
        {
            auto range = S("abcd");
            testddecodeBack(range, 'd', 1);
            testddecodeBack(range, 'c', 1);
            testddecodeBack(range, 'b', 1);
            testddecodeBack(range, 'a', 1);
        }

        {
            auto range = S("ウェブサイト");
            testddecodeBack(range, 'ト', 3);
            testddecodeBack(range, 'イ', 3);
            testddecodeBack(range, 'サ', 3);
            testddecodeBack(range, 'ブ', 3);
        }
...
@system unittest
{
                          ...
                          (wstring s) => new RefRandomCU!wchar(s)))
    {
        ...
        {
            auto range = S("ウェブサイト");
            testddecode(range, 0, 'ウ', 1);
            testddecode(range, 1, 'ェ', 2);
            testddecodeFront(range, 'ウ', 1);
            testddecodeFront(range, 'ェ', 1);
            assert(decodeFront(range) == 'ブ');
            assert(decodeFront(range) == 'サ');
        }
        ...
        {
            auto range = S("ウェブサイト");
            testddecodeBack(range, 'ト', 1);
            testddecodeBack(range, 'イ', 1);
            testddecodeBack(range, 'サ', 1);
            testddecodeBack(range, 'ブ', 1);
        }
    }
    ...
    foreach (S; AliasSeq!(to!wstring, RandomCU!wchar, (wstring s) => new RefRandomCU!wchar(s)))
    {
        ...
        testddecode(str, 0, cast(dchar) 0x10000, 2);
        testddecode(str, 2, cast(dchar) 0x1400, 3);
        testddecode(str, 3, cast(dchar) 0xB9DDE, 5);
        testddecodeBack(str, cast(dchar) 0xB9DDE, 2);
        testddecodeBack(str, cast(dchar) 0x1400, 1);
        testddecodeBack(str, cast(dchar) 0x10000, 2);
    }
    });
}
...
@system unittest
{
    ...
    foreach (S; AliasSeq!(to!dstring, RandomCU!dchar, (dstring s) => new RefRandomCU!dchar(s)))
    {
        ...
        testddecode(str, 0, 0x10000, 1);
        testddecode(str, 1, 0x1400, 2);
        testddecode(str, 2, 0xB9DDE, 3);
        testddecodeBack(str, cast(dchar) 0xB9DDE, 1);
        testddecodeBack(str, cast(dchar) 0x1400, 1);
        testddecodeBack(str, cast(dchar) 0x10000, 1);
    }
    });
}
...
/++
    ...
    Params:
        ...
        input = the $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
 ...
 * $(REF front, stdd,range,primitives) does to character arrays. As a result,
 * using ranges with `byCodeUnit` can be `nothrow` while
 * $(REF front, stdd,range,primitives) throws when it encounters invalid Unicode
 * sequences.
 *
 ...
 *
 * See_Also:
 *      Refer to the $(MREF stdd, uni) docs for a reference on Unicode
 *      terminology.
 *
 *      For a range that iterates by grapheme cluster (written character) see
 *      $(REF byGrapheme, stdd,uni).
 */
...
/****************************
 * Iterate an $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
 ...
 *      $(REF isAutodecodableString, stdd, traits), and if the base range is
