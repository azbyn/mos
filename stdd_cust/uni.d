...
/++
    $(P The $(D stdd.uni) module provides an implementation
    of fundamental Unicode algorithms and data structures.
    This doesn't include UTF encoding and decoding primitives,
    see $(REF decode, stdd,_utf) and $(REF encode, stdd,_utf) in stdd.utf
...
debug(std_uni) import stdd.stdio; // writefln, writeln
...
/**
    ...
    or the exact type of a current $(CODEPOINT) set in $(D stdd.uni).
...
public:
    ...
    */
    ...
    /**
        ...
        $(P Used by various standard formatting facilities such as
         $(REF formattedWrite, stdd,_format), $(REF write, stdd,_stdio),
         $(REF writef, stdd,_stdio), $(REF to, stdd,_conv) and others.
        )
        Example:
        ---
        import stdd.conv;
    ...
    */
    ...
    private import stdd.format : FormatException, FormatSpec;
...
template Utf8Matcher()
{
    ...
    private:
        ...
        {
            import stdd.typecons : staticIota;
            if (inp.length < size)
            {
                badEncoding();
                return false;
...
@system unittest //@@@BUG stdd.format is not @safe
{
    import stdd.format : format;
    static import stdd.ascii;
    foreach (ch; 0..0x80)
