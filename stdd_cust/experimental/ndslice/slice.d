/**
This is a submodule of $(MREF stdd, experimental, ndslice).

License:   $(HTTP www.boost.org/LICENSE_1_0.txt, Boost License 1.0).

Authors:   Ilya Yaroshenko

Source:    $(PHOBOSSRC stdd/_experimental/_ndslice/_slice.d)

Macros:
SUBREF = $(REF_ALTTEXT $(TT $2), $2, stdd,experimental, ndslice, $1)$(NBSP)
...
module stdd.experimental.ndslice.slice;
...
import stdd.experimental.ndslice.internal;
...
pure nothrow unittest
{
    ...
    import stdd.experimental.ndslice.selection : byElement;
    import stdd.range : iota;
...
pure nothrow unittest
{
    import stdd.experimental.ndslice.selection : iotaSlice;
...
nothrow unittest
{
    ...
    import stdd.experimental.allocator;
...
{
    import stdd.algorithm.comparison : equal;
    import stdd.experimental.ndslice.selection : byElement, iotaSlice;
...
@safe @nogc pure nothrow unittest
{
    import stdd.algorithm.iteration : map, sum, reduce;
    import stdd.algorithm.comparison : max;
    import stdd.experimental.ndslice.iteration : transposed;
...
See also $(MREF stdd, experimental, allocator).
Params:
    alloc = allocator
    lengths = list of lengths for each dimension
...
{
    import stdd.experimental.allocator : makeArray;
    immutable len = lengthsProduct(lengths);
...
It uses $(SUBREF selection, mapSlice) and $(REF_ALTTEXT $(TT to), to, stdd,conv)$(NBSP)
...
template as(T)
{
    ...
    auto as(size_t N, Range)(Slice!(N, Range) slice)
    {
        ...
        {
            import stdd.conv : to;
            import stdd.experimental.ndslice.selection : mapSlice;
...
unittest
{
    import stdd.experimental.ndslice.slice : as;
    import stdd.experimental.ndslice.selection : diagonal;
...
/++
Base Exception class for $(MREF stdd, experimental, ndslice).
+/
...
-------
import stdd.experimental.ndslice;
                    ...
                    || is(_Range == Slice!(N1, Range1), size_t N1, Range1)))
{
    ...
    @safe @nogc pure nothrow unittest
    {
        import stdd.experimental.ndslice.selection : pack, iotaSlice;
   ...
   +/
    ...
    {
        import stdd.experimental.ndslice.selection : pack, iotaSlice;
        import stdd.experimental.ndslice.iteration : reversed, strided, transposed;
    ...
    @safe @nogc pure nothrow unittest
    {
        import stdd.experimental.ndslice.iteration : reversed, strided, swapped;
        import stdd.experimental.ndslice.selection : iotaSlice;
        assert(iotaSlice(3, 4, 50)
            .reversed!2      //makes stride negative
    ...
    @safe @nogc pure nothrow unittest
    {
        import stdd.experimental.ndslice.selection : pack, evertPack, iotaSlice;
        ...
        if (Slice!(NR, RangeR).PureN == PureN)
    {
        ...
        foreach (i; Iota!(0, PureN))
            if (this._lengths[i] == 0)
                return true;
        import stdd.experimental.ndslice.selection : unpack;
    ...
    pure nothrow unittest
    {
        import stdd.experimental.ndslice.iteration : dropExactly;
        import stdd.experimental.ndslice.selection : iotaSlice;
    ...
    See_also: $(LREF Slice.toMurmurHash3), $(MREF stdd, _digest, murmurhash).
    +/
    ...
    unittest
    {
        import stdd.experimental.ndslice.iteration : allReversed;
        import stdd.experimental.ndslice.selection : iotaSlice;
    ...
    See_also: $(LREF Slice.toHash), $(MREF stdd, _digest, murmurhash)
    +/
    auto toMurmurHash3(uint size /* 32 or 128 */ , uint opt = size_t.sizeof == 8 ? 64 : 32)() const @trusted
    {
        import stdd.digest.murmurhash : MurmurHash3;
    ...
    static if (isMutable!DeepElemType)
    {
        ...
        pure nothrow unittest
        {
            import stdd.experimental.ndslice.selection : blocks, iotaSlice;
        ...
        pure nothrow unittest
        {
            import stdd.experimental.ndslice.selection : blocks, iotaSlice, pack;
            auto a = slice!size_t(4, 4);
        ...
        pure nothrow unittest
        {
            import stdd.experimental.ndslice.selection : blocks;
        ...
        pure nothrow unittest
        {
            import stdd.experimental.ndslice.selection : pack;
...
pure nothrow unittest
{
    import stdd.experimental.ndslice.iteration : transposed, everted;
...
See also $(MREF stdd, format).
+/
unittest
{
    import stdd.algorithm,  stdd.conv, stdd.exception, stdd.format,
        stdd.functional, stdd.string, stdd.range;
