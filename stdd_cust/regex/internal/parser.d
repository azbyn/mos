...
*/
module stdd.regex.internal.parser;

import stdd.regex.internal.ir;
import stdd.range.primitives, stdd.uni, stdd.meta,
    stdd.traits, stdd.typecons, stdd.exception;
...
auto makeRegex(S, CG)(Parser!(S, CG) p)
{
    ...
    with(re)
    {
        ...
        //@@@BUG@@@ (not reduced)
        //somehow just using validate _collides_ with stdd.utf.validate (!)
...
struct CodeGen
{
    ...
    {
        import stdd.array : insertInPlace;
    ...
    {
        static import stdd.algorithm.comparison;
        import stdd.algorithm.mutation : copy;
        import stdd.array : insertInPlace;
        immutable replace = ir[offset].code == IR.Nop;
        ...
        if (max != infinite)
        {
            if (min != 1 || max != 1)
            {
                ...
                putRaw(min);
                putRaw(max);
                counterDepth = stdd.algorithm.comparison.max(counterDepth, nesting+1);
            }
        }
    ...
    if (isForwardRange!R && is(ElementType!R : dchar))
{
    ...
    {
        uint r = 0;
        while (stdd.ascii.isDigit(current))
        {
            if (r >= (uint.max/10))
                ...
                if (current == '?')
                {
                    ...
                    case 'P':
                        ...
                        while (next() && (isAlpha(current) ||
                            current == '_' || stdd.ascii.isDigit(current)))
                        {
                            name ~= current;
                        }
                        if (current != '>')
    ...
    {//copy is @system
        ...
        case '{':
            ...
            enforce(stdd.ascii.isDigit(current), "First number required in repetition");
            min = parseDecimal();
            if (current == '}')
                max = min;
            else if (current == ',')
    ...
    void parseAtom()
    {
        ...
        case '$':
            ...
            break;
        default:
            //FIXME: getCommonCasing in new stdd uni
    ...
    {//accesses array of appender
        ...
        case '1': .. case '9':
            ...
            while (nref < maxBackref && next() && stdd.ascii.isDigit(current))
            {
        ...
        if (current == '{')
        {
            while (k < MAX_PROPERTY && next() && current !='}' && current !=':')
                if (current != '-' && current != ' ' && current != '_')
                    result[k++] = cast(char)stdd.ascii.toLower(current);
