/++
  ...
  For an introduction to $(D stdd.regex) see a
  ...
  $(I stdd.regex operates on codepoint level,
  ...
  *With exception of point 1.1.1, as of yet, normalization of input
  ...
  Authors: Dmitry Olshansky,
    ...
    API and utility constructs are modeled after the original $(D stdd.regex)
  by Walter Bright and Andrei Alexandrescu.

  Source: $(PHOBOSSRC stdd/_regex/_package.d)
...
import stdd.regex.internal.ir;
import stdd.regex.internal.thompson; //TODO: get rid of this dependency
import stdd.traits, stdd.range.primitives;
...
/++
    ...
    ----
+/
public alias Regex(Char) = stdd.regex.internal.ir.Regex!(Char);
...
+/
public alias StaticRegex(Char) = stdd.regex.internal.ir.StaticRegex!(Char);
    ...
    if (isSomeString!(S))
{
    ...
    import stdd.functional : memoize;
...
public auto regexImpl(S)(S pattern, const(char)[] flags="")
    if (isSomeString!(S))
{
    import stdd.regex.internal.parser : Parser, CodeGen;
...
{
    import stdd.regex.internal.parser, stdd.regex.internal.backtracking;
...
public:
    ...
    ///A hook for compatibility with original stdd.regex.
...
public:
    ...
    /// Same as .front, provided for compatibility with original stdd.regex.
...
{
    import stdd.regex.internal.thompson : ThompsonMatcher;
...
{
    import stdd.regex.internal.backtracking : BacktrackingMatcher;
        ...
        isOutputRange!(OutR, ElementEncodingType!(Capt.String)[]))
{
    ...
    import stdd.conv : text, parse;
    import stdd.ascii : isDigit, isAlpha;
    import stdd.exception : enforce;
...
@system unittest
{
    ...
    import stdd.array, stdd.random, stdd.conv, stdd.range;
...
public alias RegexException = stdd.regex.internal.ir.RegexException;
...
{
    import stdd.regex;
    import stdd.algorithm.comparison;
