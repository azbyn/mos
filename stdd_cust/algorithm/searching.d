...
@safe unittest
{
    ...
    assert(count!((a, b) => stdd.uni.toLower(a) == stdd.uni.toLower(b))("AbcAdFaBf", "ab") == 2);
...
/++
    ...
    Params:
        ...
        haystack = An
          $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives) of
...
find(retro(haystack), needle)). See $(REF retro, stdd,range).
...
$(D pred), call $(D find!(pred)(retro(haystack))). See $(REF retro, stdd,range).
...
haystack = The $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives) to
search in.
...
haystack = The $(REF_ALTTEXT forward range, isForwardRange, stdd,range,primitives)
searched in.

needle = The $(REF_ALTTEXT forward range, isForwardRange, stdd,range,primitives)
searched for.
...
See_Also:
    $(REF min, stdd,algorithm,comparison)
*/
...
See_Also:
    $(REF max, stdd,algorithm,comparison)
*/
...
See_Also:
    $(REF min, stdd,algorithm,comparison), $(LREF minCount), $(LREF minElement), $(LREF minPos)
 */
...
See_Also:
    $(REF max, stdd,algorithm,comparison), $(LREF maxCount), $(LREF maxElement), $(LREF maxPos)
 */
...
Params:
    ...
    r = The $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives) to skip
        over.
