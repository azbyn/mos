...
approaches have been used. $(REF make, stdd,_container,util) allows for
...
---
import stdd.container, stdd.range;
...
$(UL
    $(LI
        The $(MREF stdd, _container, array) module provides
        an array type with deterministic control of memory, not reliant on
        the GC unlike built-in arrays.
    )
    $(LI
        The $(MREF stdd, _container, binaryheap) module
        provides a binary heap implementation that can be applied to any
        user-provided random-access range.
    )
    $(LI
        The $(MREF stdd, _container, dlist) module provides
        a doubly-linked list implementation.
    )
    $(LI
        The $(MREF stdd, _container, rbtree) module
        implements red-black trees.
    )
    $(LI
        The $(MREF stdd, _container, slist) module
        implements singly-linked lists.
    )
    $(LI
        The $(MREF stdd, _container, util) module contains
...
its primary $(MREF_ALTTEXT range, stdd, range) type,
...
import stdd.algorithm.iteration : find;
import stdd.container;
...
When any $(MREF_ALTTEXT range, stdd, range) can be passed as an argument to
...
Since a sequence of elements can be removed from a $(MREF_ALTTEXT doubly linked list,stdd,_container,dlist)
in constant time, $(D DList) provides the primitive $(D c.remove(r))
as well as $(D c.linearRemove(r)). On the other hand
$(MREF_ALTTEXT Array, stdd,_container, array) only offers $(D c.linearRemove(r)).
...
Source: $(PHOBOSSRC stdd/_container/package.d)
...
module stdd.container;

public import stdd.container.array;
public import stdd.container.binaryheap;
public import stdd.container.dlist;
public import stdd.container.rbtree;
public import stdd.container.slist;
...
ranges defined in $(MREF stdd,range).
