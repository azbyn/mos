...
Example:
---
import stdd.net.curl, stdd.stdio;
...
Source: $(PHOBOSSRC stdd/net/_curl.d)
...
*/
module stdd.net.curl;

import core.thread;
import etc.c.curl;
import stdd.algorithm;
import stdd.array;
import stdd.concurrency;
import stdd.conv;
import stdd.datetime;
import stdd.encoding;
import stdd.exception;
import stdd.meta;
import stdd.regex;
import stdd.socket : InternetAddress;
import stdd.string;
import stdd.traits;
...
version(unittest)
{
    ...
    import stdd.socket : Address, INADDR_LOOPBACK, Socket, TcpSocket;
...
version(StdDdoc) import stdd.stdio;
...
unittest
{
    ...
    foreach (host; [testServer.addr, "http://"~testServer.addr])
    {
        ...
        auto fn = stdd.file.deleteme;
        scope (exit) stdd.file.remove(fn);
        download(host, fn);
        assert(stdd.file.readText(fn) == "Hello world");
    }
}
...
unittest
{
    ...
    foreach (host; [testServer.addr, "http://"~testServer.addr])
    {
        auto fn = stdd.file.deleteme;
        scope (exit) stdd.file.remove(fn);
        stdd.file.write(fn, "upload data\n");
        testServer.handle((s) {
            auto req = s.recvReq;
...
{
    // no move semantic available in stdd.concurrency ie. must use casting.
...
private mixin template Protocol()
{
    ...
    */
     ...
     * ----
     *
     * See: $(REF InternetAddress, stdd,socket)
     */
...
struct HTTP
{
    ...
    */
     ...
     * It has the form "Phobos-stdd.net.curl/$(I PHOBOS_VERSION) (libcurl/$(I CURL_VERSION))"
     */
    static string defaultUserAgent() @property
    {
        import stdd.compiler : version_major, version_minor;

        // http://curl.haxx.se/docs/versions.html
        enum fmt = "Phobos-stdd.net.curl/%d.%03d (libcurl/%d.%d.%d)";
...
/++
    Exception thrown on errors in stdd.net.curl functions.
+/
...
/++
    Exception thrown on timeout errors in stdd.net.curl functions.
+/
...
struct Curl
{
    ...
    */
    ...
    private string errorString(CurlCode code)
    {
        ...
        // doing the following (instead of just using stdd.conv.to!string) avoids 1 allocation
...
// other stdd.concurrency.receive calls does not pick up our messages
// by accident.
