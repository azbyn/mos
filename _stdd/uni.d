...
/++
    ...
    see $(REF decode, stdd,_utf) and $(REF encode, std,_utf) in std.utf
...
//@@@BUG: stdd.typecons tuples depend on std.format to produce fields mixin
    ...
    */
    ...
    /**
        ...
        $(P Used by various standard formatting facilities such as
         $(REF formattedWrite, stdd,_format), $(REF write, std,_stdio),
         $(REF writef, stdd,_stdio), $(REF to, std,_conv) and others.
        )
        Example:
        ---
        import stdd.conv;
...
private:
  ...
  package(stdd)  // used from: std.regex.internal.parser
  ...
  package(stdd)  // used from: std.regex.internal.parse
    ref add(U)(U rhs)
        if (isCodepointSet!U)
    {
        Marker start;
