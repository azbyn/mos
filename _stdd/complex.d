...
struct Complex(T)  if (isFloatingPoint!T)
{
    ...
    See the $(MREF stdd, format) and $(REF format, std,string)
...
@safe pure nothrow unittest
{
    ...
    foreach (i; 0..6)
    {
        ...
        assert (approxEqual(stdd.math.cos(arg(cei)), std.math.cos(arg(c1)*i), EPS));
    }
...
/**
    ...
    that uses $(REF _expi, stdd,math).  Unlike $(REF _expi, std,math), which uses the
...
@safe pure nothrow unittest
{
    ...
    assert(expi(1.3e5L) == complex(stdd.math.cos(1.3e5L), std.math.sin(1.3e5L)));
...
Complex!T sqrt(T)(Complex!T z)  @safe pure nothrow @nogc
{
    ...
    else
    {
        ...
        if (x >= y)
        {
            r = y / x;
            w = stdd.math.sqrt(x)
                * stdd.math.sqrt(0.5 * (1 + std.math.sqrt(1 + r * r)));
        }
        else
        {
            r = x / y;
            w = stdd.math.sqrt(y)
                * stdd.math.sqrt(0.5 * (r + std.math.sqrt(1 + r * r)));
        }
