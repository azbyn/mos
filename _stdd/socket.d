 ...
 * Source:  $(PHOBOSSRC stdd/_socket.d)
 */

module stdd.socket;

import core.stdc.stdint, core.stdc.string, stdd.string, core.stdc.stdlib, std.conv;
...
version(Windows)
{
    ...
    private import core.sys.windows.windows, stdd.windows.syserror;
...
version(unittest)
{
    ...
    private import stdd.stdio : writefln;
    ...
    private void softUnittest(void delegate() @safe test, int line = __LINE__) @trusted
    {
        ...
        {
            writefln(" --- stdd.socket(%d) test fails depending on environment ---", line);
            writefln(" (%s)", e);
        }
    }
}

/// Base exception thrown by $(D stdd.socket).
...
// stdd.socket (since it uses it as a default argument), but it probably doesn't
...
public:
    ...
    {
        return stdd.conv.to!string(port);
    }
