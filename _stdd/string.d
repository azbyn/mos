...
ptrdiff_t lastIndexOf(Char)(const(Char)[] s, in dchar c,
    ...
    if (isSomeChar!Char)
{
    static import stdd.ascii, std.uni;
    ...
    if (isSomeChar!Char1 && isSomeChar!Char2)
{
    ...
    else
    {
        for (size_t i = s.length; !s.empty;)
        {
            if (endsWith!((a, b) => stdd.uni.toLower(a) == std.uni.toLower(b))
                         (s, sub))
            {
                return cast(ptrdiff_t)i - to!(const(Char1)[])(sub).length;
            }
 ...
 *  Split an array or slicable range of characters into a range of lines
    using $(D '\r'), $(D '\n'), $(D '\v'), $(D '\f'), $(D "\r\n"),
    $(REF lineSep, stdd,uni), $(REF paraSep, std,uni) and $(D '\u0085') (NEL)
...
/++
    ...
    $(D "\r\n"), $(D '\f'), $(D '\v'), $(REF lineSep, stdd,uni), $(REF paraSep, std,uni), or $(REF nelSep, std,uni)
