...
import stdd.stdio;
import stdd.internal.processinit;
    ...
    @trusted // TODO: Should be @safe
{
    ...
    auto id = core.sys.posix.unistdd.fork();
    if (id < 0)
    ...
    void forkChild() nothrow @nogc
    {
        ...
        if (workDirFD >= 0)
        {
            if (fchdir(workDirFD) < 0)
            {
                ...
                core.sys.posix.unistdd._exit(1);
                assert(0);
            }
            close(workDirFD);
        }
        ...
        // Execute program.
        core.sys.posix.unistdd.execve(argz[0], argz.ptr, envz);
...
@system unittest // Stream redirection in spawnProcess().
{
    ...
    // Files
    import stdd.ascii, std.file, std.uuid;
    ...
    stdd.file.write(pathi, "INPUT"~std.ascii.newline);
...
@system unittest
{
    ...
    assert (wait(spawnShell(cmd, stdd.stdio.stdin, f, std.stdio.stderr, env)) == 0);
    f.close();
...
Pipe pipe() @trusted //TODO: @safe
{
    ...
    int[2] fds;
    if (core.sys.posix.unistdd.pipe(fds) != 0)
...
config    = Flags that control process creation. See $(LREF Config)
            ...
            $(D retainstdd...) flags have no effect in this function.
workDir   = The working directory for the new process.
            By default the child process inherits the parent's working
            directory.
    ...
    @trusted //TODO: @safe
{
    ...
    if (redirectFlags & Redirect.stdoutToStderr)
    {
        if (redirectFlags & Redirect.stderrToStdout)
        {
            ...
            // set, so we assign the stdd.stdio.std* streams directly.
...
@property int thisProcessID() @trusted nothrow //TODO: @safe
{
    ...
    else version (Posix) return core.sys.posix.unistdd.getpid();
}
...
@system unittest
{
    ...
    // import stdd.stdio, std.array; void main(string[] args) { write(args.join("\0")); }
...
version (Windows)
{
    import stdd.format, std.random, std.file;
}
version (Posix)
{
...
version (unittest)
{
    import stdd.file, std.conv, std.random;
}
...
else version (OSX)
{
    ...
    void browse(const(char)[] url) nothrow @nogc
    {
        ...
        auto childpid = core.sys.posix.unistdd.fork();
        if (childpid == 0)
        {
            core.sys.posix.unistdd.execvp(args[0], cast(char**)args.ptr);
