...
/++
    $(P The `stdd.uni` module provides an implementation
    of fundamental Unicode algorithms and data structures.
    This doesn't include UTF encoding and decoding primitives,
    see $(REF decode, stdd,_utf) and $(REF encode, stdd,_utf) in $(MREF stdd, utf)
...
$(TR $(TD Building blocks) $(TD
    ...
    $(P All primitives listed operate on Unicode characters and
        ...
        and ignore Unicode $(CHARACTERS), see $(MREF stdd, ascii).
...
import stdd.range.primitives; // back, ElementEncodingType, ElementType, empty,
    // front, isForwardRange, isInputRange, isRandomAccessRange, popFront, put,
    // save
import stdd.traits; // isConvertibleToString, isIntegral, isSomeChar,
    // isSomeString, Unqual
// debug = stdd_uni;

debug(stdd_uni) import stdd.stddio; // writefln, writeln
...
@safe unittest
{
    import stdd.algorithm.searching : find;
    // initialize code point sets using script/block or property name
...
{
    import stdd.algorithm.iteration : sum, map;
...
{
    import stdd.range.primitives : isOutputRange;
    ...
    {
        import stdd.format : formattedWrite;
...
template sharMethod(alias uniLowerBound)
{
        ...
        if (is(T : ElementType!Range))
    {
        ...
        import stdd.math : nextPow2, truncPow2;
...
@safe unittest
{
    import stdd.array : array;
    import stdd.range : assumeSorted, iota;

    auto stddLowerBound(T)(T[] range, T needle)
    {
...
{
    import stdd.algorithm.mutation : copy;
...
{
    import stdd.traits : isDynamicArray;
...
{
    import stdd.range.primitives : hasLength;
...
//@@@BUG: stdd.typecons tuples depend on stdd.format to produce fields mixin
// which relies on stdd.uni.isGraphical and this chain blows up with Forward reference error
// hence below doesn't seem to work
// public alias CodepointInterval = Tuple!(uint, "a", uint, "b");

/**
    The recommended type of $(REF Tuple, stdd,_typecons)
...
/**
    ...
    or the exact type of a current $(CODEPOINT) set in `stdd.uni`.
...
{
    import stdd.range : assumeSorted;
...
public:
    ...
    */
    ...
    /**
        ...
        $(P Used by various standard formatting facilities such as
         $(REF formattedWrite, stdd,_format), $(REF write, stdd,_stddio),
         $(REF writef, stdd,_stddio), $(REF to, stdd,_conv) and others.
        )
        Example:
        ---
        import stdd.conv;
    ...
    */
    ...
    private import stdd.format : FormatSpec;
    ...
    {
        import stdd.format : formatValue;
    ...
    pure @safe unittest
    {
        ...
        import stdd.format : format, FormatException;
...
private:
  ...
  package(stdd)  // used from: stdd.regex.internal.parser
  ...
  package(stdd)  // used from: stdd.regex.internal.parse
    ref add(U)(U rhs)
        if (isCodepointSet!U)
    {
        Marker start;
...
public:
    ...
    */
    @property auto inverted()
    {
        ...
        {
            inversion.addInterval(0, lastddchar+1);
            return inversion;
        }
        ...
        if (data[data.length-1] != lastddchar+1)
            genericReplace(inversion.data,
                inversion.data.length, inversion.data.length, [lastddchar+1]);
        else
            genericReplace(inversion.data,
    ...
    {
        import stdd.algorithm.searching : countUntil;
        import stdd.format : format;
        enum maxBinary = 3;
...
private:
    ...
    static struct Intervals(Range)
    {
        ...
        //may break sorted property - but we need stdd.sort to access it
    ...
    void sanitize()
    {
        ...
        import stdd.algorithm.mutation : SwapStrategy;
    ...
    do
    {
        import stdd.range : assumeSorted, SearchPolicy;
...
struct CowArray(SP=GcPolicy)
{
  ...
  @safe:
    ...
    {
        import stdd.algorithm.mutation : copy;
        import stdd.range.primitives : walkLength;
...
pure @safe unittest// Uint24 tests
{
    ...
    import stdd.algorithm.mutation : copy;
    import stdd.conv : text;
    import stdd.range : iota, chain;
    import stdd.range.primitives : isBidirectionalRange, isOutputRange;
...
{
    import stdd.algorithm.comparison : equal;
    import stdd.conv : text, to;
    import stdd.range : chain, iota;
    import stdd.typecons : tuple;
...
pure @safe unittest// iteration & opIndex
{
    import stdd.algorithm.comparison : equal;
    import stdd.conv : text;
    import stdd.typecons : tuple, Tuple;
    ...
    void spillToNextPageImpl(size_t level, Slice)(ref Slice ptr)
    {
        ...
        for (j=0; j<last; j+=pageSize)
        {
            if (ptr[j .. j+pageSize] == slice)
            {
                ...
                version(none)
                {
                import stdd.stddio : writefln, writeln;
...
if (sumOfIntegerTuple!sizes == 21)
{
    auto codepointSetTrie(Set)(Set set)
        if (isCodepointSet!Set)
    {
        auto builder = TrieBuilder!(bool, dchar, lastddchar+1, GetBitSlicing!(21, sizes))(false);
...
if (sumOfIntegerTuple!sizes == 21)
{
    ...
    alias CodepointSetTrie = typeof(TrieBuilder!(bool, dchar, lastddchar+1, Prefix)(false).build());
}
...
{
    alias Prefix = GetBitSlicing!(21, sizes);
    alias CodepointTrie = typeof(TrieBuilder!(T, dchar, lastddchar+1, Prefix)(T.init).build());
}
...
if (isValidArgsForTrie!(Key, Args))
{
    ...
    /*
        ...
        See_Also: $(REF sort, stdd,_algorithm),
        $(REF SortedRange, stdd,range),
        $(REF setUnion, stdd,_algorithm).
    */
    ...
    */
    ...
    auto buildTrie(Range)(Range range, Value filler, bool unsorted)
        if (isInputRange!Range
            ...
            && is(typeof(Range.init.front[1]) : Key))
    {
        import stdd.algorithm.sorting : multiSort;
...
template Utf8Matcher()
{
    ...
    static auto encode(size_t sz)(dchar ch)
        if (sz > 1)
    {
        import stdd.utf : encodeUTF = encode;
        char[4] buf;
        encodeUTF(buf, ch);
        char[sz] ret;
        buf[0] &= leadMask!sz;
    ...
    auto build(Set)(Set set)
    {
        ...
        auto utf8_4 = set & CodepointSet(0x1_0000, lastddchar+1);
    ...
    {
        import stdd.format : format;
        import stdd.meta : Erase, staticIndexOf;
    ...
    struct Impl(Sizes...)
    {
        import stdd.meta : allSatisfy, staticMap;
...
{
    import stdd.algorithm.iteration : map;
    import stdd.exception : collectException;
    import stdd.format : format;
    auto utf16 = utfMatcher!wchar(unicode.L);
    auto utf8 = utfMatcher!char(unicode.L);
...
/**
    ...
    for algorithms like stdd.algorithm.find that take unary predicates. )
...
{
    import stdd.algorithm.comparison : cmp;
    import stdd.algorithm.iteration : map, filter;
    import stdd.ascii : toLower;
...
if (isInputRange!Range && isIntegralPair!(ElementType!Range))
{
    ...
    foreach (val; intervals)
    {
        ...
        base = val[0];
        if (val[1] != lastddchar+1) // till the end of the domain so don't store it
        {
...
@safe pure unittest
{
    ...
    auto run2 = [tuple(0, (1 << 20)+512+1), tuple((1 << 20)+512+4, lastddchar+1)];
...
{
    import stdd.internal.unicode_tables : uniProps; // generated file
...
{
    import stdd.exception : enforce;
    //stdd.conv.parse is both @system and bogus
    uint val;
...
@safe unittest
{
    import stdd.algorithm.searching : canFind;
    import stdd.exception : collectException;
...
struct UnicodeSetParser(Range)
{
    ...
    CodepointSet parseSet()
    {
        ValStack vstack;
        OpStack opstack;
        import stdd.functional : unaryFun;
...
@safe public struct unicode
{
    ...
    */
    struct block
    {
        import stdd.internal.unicode_tables : blocks; // generated file
    ...
    */
    struct script
    {
        import stdd.internal.unicode_tables : scripts; // generated file
    ...
    */
    ...
    {
        import stdd.internal.unicode_tables : hangul; // generated file
            ...
            if (front == '{')
            {
                    ...
                    && front !=':')
                {
                    if (front != '-' && front != ' ' && front != '_')
                        result[k++] = cast(char) stdd.ascii.toLower(front);
                    popFront();
                }
...
private:
    ...
    {
        import stdd.internal.unicode_tables : blocks, scripts, uniProps; // generated file
    ...
    {
        import stdd.conv : to;
        import stdd.internal.unicode_tables : blocks, scripts; // generated file
        Set set;
...
@safe unittest
{
    import stdd.internal.unicode_tables : blocks, uniProps; // generated file
...
template genericDecodeGrapheme(bool getValue)
{
    ...
    {
        import stdd.internal.unicode_tables : isHangL, isHangT, isHangV; // generated file
        enum GraphemeState {
            Start,
            CR,
            RI,
...
/++
    ...
    $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives) of dchar `inp`.
...
@system unittest
{
    ...
    import stdd.range : isRandomAccessRange;
...
/++
    ...
    Params:
        r1 = an $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives) of characters
        r2 = an $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives) of characters
    ...
    See_Also:
        $(LREF icmp)
        $(REF cmp, stdd,algorithm,comparison)
+/
...
{
    import stdd.internal.unicode_tables : sTable = simpleCaseTable; // generated file
    import stdd.utf : decodeFront;
    import stdd.typecons : Yes;
    static import stdd.ascii;
    ...
    {{
        ...
        for (; i < end; ++i)
        {
            ...
            if (lhs == rhs) continue;
            auto lowDiff = stdd.ascii.toLower(lhs) - stdd.ascii.toLower(rhs);
    ...
    while (!r1.empty)
    {
        ...
        if ((lhs | rhs) < 0x80)
        {
            immutable d = stdd.ascii.toLower(lhs) - stdd.ascii.toLower(rhs);
            if (!d) continue;
            return d;
        }
...
{
    import stdd.algorithm.searching : skipOver;
    import stdd.internal.unicode_tables : fullCaseTable; // generated file
...
/**
 * By using $(REF byUTF, stdd,utf) and its aliases, GC allocations via auto-decoding
...
@safe unittest
{
    import stdd.algorithm.sorting : sort;
    import stdd.conv : to;
    import stdd.exception : assertCTFEable;
    assertCTFEable!(
    {
...
@safe pure unittest
{
    import stdd.algorithm.iteration : joiner, map;
    import stdd.algorithm.sorting : sort;
    import stdd.array : array;
    auto a = [["foo", "bar"], ["baz"]].map!(line => line.joiner(" ")).array.sort!((a, b) => icmp(a, b) < 0);
}

// This is package for the moment to be used as a support tool for stdd.regex
...
{
    import stdd.internal.unicode_tables : simpleCaseTable; // generated file
...
{
    import stdd.algorithm.iteration : map;
    import stdd.internal.unicode_comp : compositionTable, composeCntShift, composeIdxMask;
    import stdd.range : assumeSorted;
...
{
    import stdd.algorithm.searching : until;
    import stdd.internal.unicode_decomp : decompCompatTable, decompCanonTable;
...
@system unittest
{
    ...
    static void testddecomp(UnicodeDecomposition T)(dchar ch, string r)
    {
        Grapheme g = decompose!T(ch);
        assert(equal(g[], r), text(g[], " vs ", r));
    }
    testddecomp!Canonical('\u1FF4', "\u03C9\u0301\u0345");
    testddecomp!Canonical('\uF907', "\u9F9C");
    testddecomp!Compatibility('\u33FF', "\u0067\u0061\u006C");
    testddecomp!Compatibility('\uA7F9', "\u0153");
...
inout(C)[] normalize(NormalizationForm norm=NFC, C)(inout(C)[] input)
{
    ...
    do
    {
        ...
        {
            import stdd.algorithm.mutation : remove;
...
{
    import stdd.internal.unicode_tables; // : toLowerTable, toTitleTable, toUpperTable; // generated file
...
{
    import stdd.internal.unicode_tables : isWhiteGen; // generated file
...
{
    import stdd.ascii : isLower, isASCII;
    if (isASCII(c))
        return isLower(c);
...
{
    import stdd.ascii : isUpper, isASCII;
    if (isASCII(c))
        return isUpper(c);
...
/*********************
 * Convert an $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
 * or a string to upper or lower case.
 *
 * Does not allocate memory.
 * Characters in UTF-8 or UTF-16 format that cannot be decoded
 * are treated as $(REF replacementDchar, stdd,utf).
 *
 * Params:
    ...
    !isConvertibleToString!Range)
{
    ...
    {
        static import stdd.ascii;
        return toCaser!(LowerTriple, stdd.ascii.toLower)(str);
    }
}
    ...
    !isConvertibleToString!Range)
{
    ...
    {
        static import stdd.ascii;
        return toCaser!(UpperTriple, stdd.ascii.toUpper)(str);
    }
}
...
{
    import stdd.traits : StringTypeOf;
...
/*********************
 * Capitalize an $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
...
// TODO: helper, I wish stdd.utf was more flexible (and stright)
...
{
    import stdd.utf : decode, codeLength;
    size_t curIdx = 0;
    size_t destIdx = 0;
...
{
    static import stdd.ascii;
    return toCase!(LowerTriple, stdd.ascii.toLower)(s);
}
// overloads for the most common cases to reduce compile time
@safe pure /*TODO nothrow*/
{
...
@safe unittest
{
    ...
    import stdd.format : format;
    foreach (ch; 0 .. 0x80)
        assert(stdd.ascii.toLower(ch) == toLower(ch));
    assert(toLower('Я') == 'я');
    assert(toLower('Δ') == 'δ');
...
/++
    ...
    toUpper can be used as an argument to $(REF map, stdd,algorithm,iteration)
    to produce an algorithm that can convert a range of characters to upper case
    without allocating memory.
    A string can then be produced by using $(REF copy, stdd,algorithm,mutation)
    to send it to an $(REF appender, stdd,array).
+/
@safe pure nothrow @nogc
...
@safe unittest
{
    ...
    import stdd.format : format;
    foreach (ch; 0 .. 0x80)
        assert(stdd.ascii.toUpper(ch) == toUpper(ch));
    assert(toUpper('я') == 'Я');
    assert(toUpper('δ') == 'Δ');
...
{
    static import stdd.ascii;
    return toCase!(UpperTriple, stdd.ascii.toUpper)(s);
}
// overloads for the most common cases to reduce compile time
@safe pure /*TODO nothrow*/
{
...
bool isAlphaNum(dchar c)
{
    ...
    // optimization for ascii case
    if (stdd.ascii.isASCII(c))
    {
        return stdd.ascii.isAlphaNum(c);
    }
    else
    {
...
bool isPunctuation(dchar c)
{
    ...
    if (c <= 0x7F)
    {
        return stdd.ascii.isPunctuation(c);
    }
    else
    {
...
{
    import stdd.internal.unicode_tables : isSpaceGen; // generated file
    return isSpaceGen(c);
}
...
{
    import stdd.internal.unicode_tables : isControlGen; // generated file
...
{
    import stdd.internal.unicode_tables : isFormatGen; // generated file
...
{
    import stdd.internal.unicode_tables; // generated file
    ...
    auto nfcQCTrie()
    {
        import stdd.internal.unicode_norm : nfcQCTrieEntries;
    ...
    auto nfdQCTrie()
    {
        import stdd.internal.unicode_norm : nfdQCTrieEntries;
    ...
    auto nfkcQCTrie()
    {
        import stdd.internal.unicode_norm : nfkcQCTrieEntries;
    ...
    auto nfkdQCTrie()
    {
        import stdd.internal.unicode_norm : nfkdQCTrieEntries;
    ...
    auto mcTrie()
    {
        import stdd.internal.unicode_grapheme : mcTrieEntries;
    ...
    {
        import stdd.internal.unicode_grapheme : graphemeExtendTrieEntries;
    ...
    auto hangLV()
    {
        import stdd.internal.unicode_grapheme : hangulLVTrieEntries;
    ...
    auto hangLVT()
    {
        import stdd.internal.unicode_grapheme : hangulLVTTrieEntries;
    ...
    {
        import stdd.internal.unicode_comp : combiningClassTrieEntries;
    ...
    {
        import stdd.internal.unicode_decomp : compatMappingTrieEntries;
    ...
    {
        import stdd.internal.unicode_decomp : canonMappingTrieEntries;
    ...
    {
        import stdd.internal.unicode_comp : compositionJumpTrieEntries;
