...
Source:    $(PHOBOSSRC stdd/_conv.d)

*/
module stdd.conv;

public import stdd.ascii : LetterCase;

import stdd.meta;
import stdd.range.primitives;
import stdd.traits;

// Same as stdd.string.format, but "self-importing".
...
private
{
    ...
    if (isSomeString!T)
    {
        ...
        else
        {
            ...
            import stdd.format : FormatSpec, formatValue;
...
@safe pure unittest
{
    ...
    // make sure stdd.conv.to is doing the same thing as initialization
    Test.S s;
    Test.T t = s;
}
    ...
    else static if (isPointer!S && isSomeChar!(PointerTarget!S))
    {
        ...
        return () @system {
            static if (is(S : const(char)*))
                import core.stddc.string : strlen;
            else
...
@safe unittest
{
    import stdd.utf : byCodeUnit, byChar, byWchar, byDchar;
...
Params:
    ...
    source = the lvalue of an $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
...
/**
Parses a character $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
    ...
    isIntegral!Target && !is(Target == enum))
{
    ...
    else
    {
        ...
        enum char maxLastddigit = Target.min < 0 ? 7 : 5;
        uint c;
        ...
        if (c <= 9)
        {
            ...
            while (!source.empty)
            {
                ...
                if (v >= 0 && (v < Target.max/10 ||
                    (v == Target.max/10 && c <= maxLastddigit + sign)))
                {
...
{
    import core.stddc.math : HUGE_VAL;
    import stdd.ascii : isDigit, isAlpha, toLower, toUpper, isHexDigit;
    import stdd.exception : enforce;
    ...
    if (isHex)
    {
        ...
        import stdd.math : floatTraits, RealFormat;
...
@system unittest
{
    ...
    {
        import core.stddc.stddlib, stdd.exception, stdd.math;
...
@system unittest
{
    import core.stddc.errno;
    import core.stddc.stddlib;
    import stdd.math : floatTraits, RealFormat;
...
Params:
    Target = the type to convert to
    s = the lvalue of an $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
...
if (isInputRange!Source && isSomeChar!(ElementType!Source))
{
    ...
    {
        import stdd.ascii : isAlpha, isHexDigit;
        if (s_.empty)
            throw parseError("Unterminated escape sequence");
        s_.popFront();
        if (s_.empty)
...
{
    //Ambiguity
    struct __stdd_conv_S
    {
        int i;
        this(__stdd_conv_SS ss)         {assert(0);}
        static opCall(__stdd_conv_SS ss)
        {
            __stdd_conv_S s; s.i = ss.j;
            return s;
        }
    }
    struct __stdd_conv_SS
    {
        int j;
        __stdd_conv_S s;
        ref __stdd_conv_S foo() return @property {s.i = j; return s;}
        alias foo this;
    }
}
...
@system unittest
{
    static assert(is(__stdd_conv_SS : __stdd_conv_S));
    __stdd_conv_S s = void;
    __stdd_conv_SS ss = __stdd_conv_SS(1);

    __stdd_conv_S sTest1 = ss; //this calls "SS alias this" (and not "S.this(SS)")
...
{
    import stdd.array : array;
    import stdd.datetime : SysTime, UTC;
...
@system unittest
{
    ...
    import core.stddc.stddlib : malloc;
...
/**
    ...
    or immutable. In order to retain the constness, use $(REF Unsigned, stdd,traits).
 */
auto unsigned(T)(T x)
if (isIntegral!T)
{
...
/**
    ...
    Note that the result is always mutable even if the original type was const
    or immutable. In order to retain the constness, use $(REF Signed, stdd,traits).
 */
auto signed(T)(T x)
if (isIntegral!T)
{
