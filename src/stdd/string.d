...
$(LEADINGROW Publicly imported functions)
    $(TR $(TD stdd.algorithm)
        $(TD
         $(REF_SHORT cmp, stdd,algorithm,comparison)
         $(REF_SHORT count, stdd,algorithm,searching)
         $(REF_SHORT endsWith, stdd,algorithm,searching)
         $(REF_SHORT startsWith, stdd,algorithm,searching)
    ))
    $(TR $(TD stdd.array)
        $(TD
         $(REF_SHORT join, stdd,array)
         $(REF_SHORT replace, stdd,array)
         $(REF_SHORT replaceInPlace, stdd,array)
         $(REF_SHORT split, stdd,array)
         $(REF_SHORT empty, stdd,array)
    ))
    $(TR $(TD stdd.format)
        $(TD
         $(REF_SHORT format, stdd,format)
         $(REF_SHORT sformat, stdd,format)
    ))
    $(TR $(TD stdd.uni)
        $(TD
         $(REF_SHORT icmp, stdd,uni)
         $(REF_SHORT toLower, stdd,uni)
         $(REF_SHORT toLowerInPlace, stdd,uni)
         $(REF_SHORT toUpper, stdd,uni)
         $(REF_SHORT toUpperInPlace, stdd,uni)
    ))
)

There is a rich set of functions for _string handling defined in other modules.
Functions related to Unicode and ASCII are found in $(MREF stdd, uni)
and $(MREF stdd, ascii), respectively. Other functions that have a
wider generality than just strings can be found in $(MREF stdd, algorithm)
and $(MREF stdd, range).

See_Also:
    $(LIST
    $(MREF stdd, algorithm) and
    $(MREF stdd, range)
...
Source:    $(PHOBOSSRC stdd/_string.d)
...
public import stdd.format : format, sformat;
import stdd.typecons : Flag, Yes, No;
public import stdd.uni : icmp, toLower, toLowerInPlace, toUpper, toUpperInPlace;

import stdd.meta; // AliasSeq, staticIndexOf
import stdd.range.primitives; // back, ElementEncodingType, ElementType, front,
    ...
    // save;
import stdd.traits; // isConvertibleToString, isNarrowString, isSomeChar,
    // isSomeString, StringTypeOf, Unqual

//public imports for backward compatibility
public import stdd.algorithm.comparison : cmp;
public import stdd.algorithm.searching : startsWith, endsWith, count;
public import stdd.array : join, replace, replaceInPlace, split, empty;

/* ************* Exceptions *************** */

/++
    Exception thrown on errors in stdd.string functions.
  +/
class StringException : Exception
{
    import stdd.exception : basicExceptionCtors;
...
/++
    ...
    Throws:
        ...
        formed codepoint, then a $(REF UTFException, stdd,utf) may be thrown.

    See_Also: $(REF countUntil, stdd,algorithm,searching)
  +/
...
if (isInputRange!Range && isSomeChar!(ElementType!Range))
{
    ...
    static import stdd.uni;
    import stdd.utf : byDchar, byCodeUnit, UTFException, codeLength;
    ...
    if (cs == Yes.caseSensitive)
    {
        ...
        {
            if (stdd.ascii.isASCII(c) && !__ctfe)
            {                                               // Plain old ASCII
    ...
    else
    {
        ...
        {                                                   // Plain old ASCII
            immutable c1 = cast(char) stdd.ascii.toLower(c);

            ptrdiff_t i;
            foreach (const c2; s.byCodeUnit())
            {
                if (c1 == stdd.ascii.toLower(c2))
                    return i;
                ++i;
            }
        }
        ...
        {                                                   // c is a universal character
            immutable c1 = stdd.uni.toLower(c);

            ptrdiff_t i;
            foreach (const c2; s.byDchar())
            {
                if (c1 == stdd.uni.toLower(c2))
                    return i;
                i += codeLength!Char(c2);
            }
        }
...
@safe pure unittest
{
    assert(testAliasedString!indexOf("stdd/string.d", '/'));

    enum S : string { a = "stdd/string.d" }
...
@safe pure unittest
{
    ...
    import stdd.utf : byChar, byWchar, byDchar;
...
@safe pure unittest
{
    assert(testAliasedString!indexOf("stdd/string.d", '/', 0));
    assert(testAliasedString!indexOf("stdd/string.d", '/', 1));
    assert(testAliasedString!indexOf("stdd/string.d", '/', 4));
...
@safe pure unittest
{
    import stdd.conv : to;
    import stdd.traits : EnumMembers;
    import stdd.utf : byCodeUnit, byChar, byWchar;
    ...
    isSomeChar!Char)
{
    ...
    else
    {
        ...
        import stdd.utf : byDchar, codeLength;
...
@safe pure unittest
{
    assert(testAliasedString!indexOf("stdd/string.d", "string"));
}
...
if (isSomeChar!Char)
{
    static import stdd.ascii, stdd.uni;
    import stdd.utf : canSearchInCodeUnits;
    if (cs == Yes.caseSensitive)
    {
    ...
    else
    {
        if (stdd.ascii.isASCII(c))
        {
            immutable c1 = stdd.ascii.toLower(c);

            foreach_reverse (i, it; s)
            {
                immutable c2 = stdd.ascii.toLower(it);
                if (c1 == c2)
                {
                    return i;
                }
        ...
        else
        {
            ...
            foreach_reverse (i, dchar it; s)
            {
                immutable c2 = stdd.uni.toLower(it);
                if (c1 == c2)
                {
                    return i;
                }
...
if (isSomeChar!Char1 && isSomeChar!Char2)
{
    import stdd.algorithm.searching : endsWith;
    import stdd.conv : to;
    import stdd.range.primitives : walkLength;
    static import stdd.uni;
    import stdd.utf : strideBack;
    if (sub.empty)
        return -1;
    ...
    else
    {
        for (size_t i = s.length; !s.empty;)
        {
            if (endsWith!((a, b) => stdd.uni.toLower(a) == stdd.uni.toLower(b))
                         (s, sub))
            {
                return cast(ptrdiff_t) i - to!(const(Char1)[])(sub).length;
            }
...
{
    import stdd.algorithm.searching : canFind, findAmong;
    if (cs == Yes.caseSensitive)
    {
        static if (forward)
        {
...
/**
    ...
    does not represent a well formed codepoint, then a $(REF UTFException, stdd,utf)
    may be thrown.
...
/**
    ...
    represent a well formed codepoint, then a $(REF UTFException, stdd,utf) may be
    thrown.
...
if (isSomeChar!Char)
{
    import stdd.traits : ModifyTypePreservingTQ;
 ...
 *
 * See_Also:
 *      $(REF asCapitalized, stdd,uni) for a lazy range version that doesn't allocate memory
 */
S capitalize(S)(S input) @trusted pure
if (isSomeString!S)
{
    import stdd.array : array;
    import stdd.uni : asCapitalized;
    import stdd.utf : byUTF;
...
/++
    ...
    $(D '\r'), $(D '\n'), $(D "\r\n"), $(REF lineSep, stdd,uni),
    $(REF paraSep, stdd,uni), $(D U+0085) (NEL), $(D '\v')  and $(D '\f')
  ...
  See_Also:
    $(LREF lineSplitter)
    $(REF splitter, stdd,algorithm)
    $(REF splitter, stdd,regex)
 +/
...
if (isSomeChar!C)
{
    ...
    import stdd.uni : lineSep, paraSep;
 ...
 *  Split an array or slicable range of characters into a range of lines
    using $(D '\r'), $(D '\n'), $(D '\v'), $(D '\f'), $(D "\r\n"),
    $(REF lineSep, stdd,uni), $(REF paraSep, stdd,uni) and $(D '\u0085') (NEL)
...
{
    import stdd.algorithm.comparison : equal;
    import stdd.range : only;

    auto s = "stdd/string.d";
...
/++
    Strips leading whitespace (as defined by $(REF isWhite, stdd,uni)) or
    as specified in the second argument.

    Params:
        input = string or $(REF_ALTTEXT forward range, isForwardRange, stdd,range,primitives)
        of characters
    ...
    will share the same tail (see $(REF sameTail, stdd,array)).

    See_Also:
        Generic stripping on ranges: $(REF _stripLeft, stdd, algorithm, mutation)
  +/
...
/++
    Strips trailing whitespace (as defined by $(REF isWhite, stdd,uni)) or
    as specified in the second argument.
    ...
    See_Also:
        Generic stripping on ranges: $(REF _stripRight, stdd, algorithm, mutation)
  +/
...
@safe pure unittest
{
    import stdd.array : array;
    import stdd.uni : lineSep, paraSep;
    import stdd.utf : byChar, byDchar, byUTF, byWchar, invalidUTFstrings;
...
{
    import stdd.array : array;
    import stdd.utf : byChar, byDchar, byUTF, byWchar;
...
/++
    ...
    $(REF isWhite, stdd,uni)) or as specified in the second argument.
    ...
    See_Also:
        Generic stripping on ranges: $(REF _strip, stdd, algorithm, mutation)
  +/
...
@safe pure unittest
{
    import stdd.array : sameHead, sameTail;
    import stdd.exception : assertCTFEable;
    assertCTFEable!(
    {
    wstring s = " ";
...
/++
    ...
    $(D "\r\n"), $(D '\f'), $(D '\v'), $(REF lineSep, stdd,uni), $(REF paraSep, stdd,uni), or $(REF nelSep, stdd,uni)
...
if ((isRandomAccessRange!Range && isSomeChar!(ElementEncodingType!Range) ||
    ...
    !isConvertibleToString!Range)
{
    import stdd.uni : lineSep, paraSep, nelSep;
    if (str.empty)
        return str;
...
/++
    ...
    Params:
        str = string or $(REF_ALTTEXT forward range, isForwardRange, stdd,range,primitives)
        of characters
    ...
    isSomeChar!C2)
{
    ...
    static if (is(Unqual!C1 == Unqual!C2) && (isSomeString!Range || (hasSlicing!Range && C2.sizeof == 4)))
    {
        import stdd.algorithm.searching : startsWith;
...
{
    import stdd.array : array;
    import stdd.utf : byChar, byWchar, byDchar, byCodeUnit, invalidUTFstrings;
    ...
    !isConvertibleToString!Range)
{
    ...
    static if (C.sizeof == 1)
    {
        import stdd.utf : byDchar, byChar;
        return leftJustifier(r.byDchar, width, fillChar).byChar;
    }
    else static if (C.sizeof == 2)
    {
        import stdd.utf : byDchar, byWchar;
...
/++
    ...
    Params:
        r = string or $(REF_ALTTEXT forward range, isForwardRange, stdd,range,primitives)
        of characters
        width = minimum field width
        fillChar = used to pad end up to $(D width) characters
    ...
    !isConvertibleToString!Range)
{
    ...
    {
        import stdd.range : chain, repeat;
        import stdd.range.primitives : walkLength;
...
{
    import stdd.uni : lineSep, paraSep, nelSep;
    import stdd.utf : codeUnitLimit, decodeFront;
...
{
    import stdd.array : array;
    import stdd.utf : byChar, byWchar;
...
/++
    ...
    See_Also:
        $(LREF tr),
        $(REF replace, stdd,array),
        $(REF substitute, stdd,algorithm,iteration)
 ...
 * Please use the functions in $(MREF stdd, regex) and $(MREF stdd, algorithm)
...
/***************************************************************
 ...
 $(REF inPattern, stdd,string)). Updates $(D_PARAM s =
...
if (isSomeString!S ||
    ...
    !isInfinite!S))
{
    import stdd.algorithm.comparison : among;
    import stdd.ascii : isASCII;
...
@safe pure nothrow unittest
{
    ...
    import stdd.utf : byChar, byDchar, byWchar;
