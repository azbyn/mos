...
$(TR $(TD Other) $(TD
    ...
    Source:    $(PHOBOSSRC stdd/_exception.d)

 +/
module stdd.exception;
...
/++
    ...
    assert("%s".format().ifThrown!Exception(e => e.classinfo.name) == "stdd.format.FormatException");
    --------------------
    )
    +/
//lazy version
...
@system unittest
{
    ...
    //Use a lambda to get the thrown object.
    assert("%s".format().ifThrown(e => e.classinfo.name) == "stdd.format.FormatException");
}
...
$(REF Take, stdd,range) when sliced with a specific lower and upper
bound (see $(REF hasSlicing, stdd,range,primitives)); $(D handle) deals with
...
if (isInputRange!Range)
{
    static struct Handler
    {
        ...
        static if (hasSlicing!Range)
        {
            ...
            {
                import stdd.range : Take, takeExactly;
