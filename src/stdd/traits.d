 ...
 *            Kenji Hara,
 *            Shoichi Kato
 * Source:    $(PHOBOSSRC stdd/_traits.d)
 */
...
import stdd.meta : AliasSeq, allSatisfy;
import stdd.functional : unaryFun;

// Legacy inheritance from stdd.typetuple
// See also: https://github.com/dlang/phobos/pull/5484#discussion_r122602797
import stdd.meta : staticMapMeta = staticMap;
// TODO: find a way to trigger deprecation warnings
//deprecated("staticMap is part of stdd.meta: Please import stdd.meta")
...
// Petit demangler
// (this or similar thing will eventually go to stdd.demangle if necessary
...
@safe unittest
{
    static assert(packageName!packageName == "stdd");
}
...
@safe unittest
{
    ...
    // static assert(packageName!stdd == "stdd");  // this package (currently: "stdd.stdd")
    static assert(packageName!(stdd.traits) == "stdd");     // this module
    static assert(packageName!packageName == "stdd");      // symbol in this module
    static assert(packageName!(stdd.array) == "stdd");  // other module from same package
    ...
    struct X12287(T) { T i; }
    static assert(packageName!(X12287!int.i) == "stdd");
}
...
@safe unittest
{
    static assert(moduleName!moduleName == "stdd.traits");
}
...
@safe unittest
{
    ...
    static assert(!__traits(compiles, moduleName!stdd));
    static assert(moduleName!(stdd.traits) == "stdd.traits");            // this module
    static assert(moduleName!moduleName == "stdd.traits");              // symbol in this module
    static assert(moduleName!(stdd.array) == "stdd.array");      // other module
    static assert(moduleName!(stdd.array.array) == "stdd.array");  // symbol in other module
    ...
    struct X12287(T) { T i; }
    static assert(moduleName!(X12287!int.i) == "stdd.traits");
}
...
@safe unittest
{
    static assert(fullyQualifiedName!fullyQualifiedName == "stdd.traits.fullyQualifiedName");
}
...
private template fqnSym(alias T)
{
    ...
    {
        import stdd.algorithm.searching : findSplit, skipOver;
...
@safe unittest
{
    ...
    static assert(fqn!fqn == "stdd.traits.fullyQualifiedName");

    alias qnTests = QualifiedNameTests;
    enum prefix = "stdd.traits.QualifiedNameTests.";
...
@safe unittest
{
    ...
    // Main tests
    enum inner_name = "stdd.traits.QualifiedNameTests.Inner";
    with (QualifiedNameTests)
    {
        ...
        static assert(fqn!(QualifiedEnum) == "stdd.traits.QualifiedEnum"); // type
        static assert(fqn!(QualifiedEnum.a) == "stdd.traits.QualifiedEnum.a"); // symbol
...
{
    import stdd.meta : anySatisfy, Filter;
...
Note:
 ...
 you should use the $(REF NoDuplicates, stdd,meta) template to avoid
...
$(D stdd.range.ElementType!Range) in the case of narrow strings, or if T
...
{
    import stdd.meta : Alias, AliasSeq, Filter;
...
@safe unittest
{
    ...
    import stdd.internal.test.uda : Attr, HasPrivateMembers;
