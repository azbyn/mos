/**
This module is a submodule of $(MREF stdd, range).

The main $(MREF stdd, range) module provides template-based tools for working with
...
wrap around range objects created by the $(MREF stdd, range) templates.
...
$(BOOKTABLE ,
    ...
    $(TR $(TD $(LREF MostdderivedInputRange))
        $(TD Returns the interface type that best matches the range.
    ))
)


Source: $(PHOBOSSRC stdd/range/_interfaces.d)
 ...
 * $(REF_ALTTEXT isInputRange, isInputRange, stdd, range, primitives)
...
template MostdderivedInputRange(R)
if (isInputRange!(Unqual!R))
{
    ...
    static if (isRandomAccessRange!R)
    {
        ...
        {
            alias MostdderivedInputRange = RandomAccessInfinite!E;
        }
        else static if (hasAssignableElements!R)
        {
            alias MostdderivedInputRange = RandomFiniteAssignable!E;
        }
        else
        {
            alias MostdderivedInputRange = RandomAccessFinite!E;
        }
    }
    else static if (isBidirectionalRange!R)
    {
        ...
        {
            alias MostdderivedInputRange = BidirectionalAssignable!E;
        }
        else
        {
            alias MostdderivedInputRange = BidirectionalRange!E;
        }
    }
    else static if (isForwardRange!R)
    {
        static if (hasAssignableElements!R)
        {
            alias MostdderivedInputRange = ForwardAssignable!E;
        }
        else
        {
            alias MostdderivedInputRange = ForwardRange!E;
        }
    }
    else
    {
        static if (hasAssignableElements!R)
        {
            alias MostdderivedInputRange = InputAssignable!E;
        }
        else
        {
            alias MostdderivedInputRange = InputRange!E;
        }
    }
}
...
if (isInputRange!(Unqual!R))
{
    ...
    else
    {
        ...
        ///
        class InputRangeObject : MostdderivedInputRange!(R) {
            private R _range;
            private alias E = ElementType!R;
