...
Source: $(PHOBOSSRC stdd/algorithm/_mutation.d)

Macros:
T2=$(TR $(TDNW $(LREF $1)) $(TD $+))
 */
module stdd.algorithm.mutation;

import stdd.range.primitives;
import stdd.traits : isArray, isAssignable, isBlitAssignable, isNarrowString, Unqual, isSomeChar;
// FIXME
...
Params:
    front = an $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
    back = a $(REF_ALTTEXT forward range, isForwardRange, stdd,range,primitives)
...
@safe unittest
{
    ...
    import stdd.range.primitives : popFrontN;
...
if (isInputRange!InputRange && isForwardRange!ForwardRange)
{
    import stdd.array : sameHead;
    import stdd.range : take, Take;
...
Params:
    source = an $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
...
/**
$(REF retro, stdd,range) can be used to achieve behavior similar to
$(LINK2 http://en.cppreference.com/w/cpp/algorithm/copy_backward, STL's `copy_backward`'):
*/
@safe unittest
{
    import stdd.algorithm, stdd.range;
    int[] src = [1, 2, 4];
    int[] dest = [0, 0, 0, 0, 0];
...
a `filter` $(REF_ALTTEXT forward _range, isForwardRange, stdd,_range,primitives)
can be provided. The length of `filler` and `range` do not need to match, but
`filler` must not be empty.

Params:
        range = An
                $(REF_ALTTEXT input _range, isInputRange, stdd,_range,primitives)
                that exposes references to its elements and has assignable
                elements
        value = Assigned to each element of range
        filler = A
                $(REF_ALTTEXT forward _range, isForwardRange, stdd,_range,primitives)
...
if (isInputRange!Range && hasLvalueElements!Range && hasAssignableElements!Range)
{
    ...
    import stdd.traits : hasElaborateAssign, isDynamicArray;
...
{
    import stdd.traits : hasElaborateDestructor;
...
void moveEmplace(T)(ref T source, ref T target) @system
{
    ...
    import stdd.traits : hasAliasing, hasElaborateAssign,
                        hasElaborateCopyConstructor, hasElaborateDestructor,
                        isAssignable;

    static if (!is(T == class) && hasAliasing!T) if (!__ctfe)
    {
        import stdd.exception : doesPointTo;
...
Params:
    src = An $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives) with
        movable elements.
    tgt = An $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives) with
...
{
    import stdd.stdio;
    import stdd.algorithm.sorting : partition;
    int[] a = [0, 1, 2, 3];
...
length. This is because all functions in `stdd.algorithm` only change $(I
...
Params:
    ...
    range = a $(REF_ALTTEXT bidirectional range, isBidirectionalRange, stdd,_range,primitives)
...
$(REF_ALTTEXT bidirectional range, isBidirectionalRange, stdd,_range,primitives) `range` by removing
...
@safe unittest
{
    ...
    import stdd.algorithm.searching : all, any;
    import stdd.algorithm.sorting : isStrictlyMonotonic;
    import stdd.array : array;
    import stdd.meta : AliasSeq;
    import stdd.range : iota, only;
    import stdd.typecons : Tuple;
    alias S = Tuple!(int[2]);
    S[] soffsets;
    foreach (start; 0 .. 5)
...
Params:
    r = a $(REF_ALTTEXT bidirectional range, isBidirectionalRange, stdd,range,primitives)
...
See_Also: $(REF retro, stdd,range) for a lazy reverse without changing `r`
*/
...
/**
    ...
    Params:
        range = a $(REF_ALTTEXT bidirectional range, isBidirectionalRange, stdd,range,primitives)
        or $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
...
{
    import stdd.traits : hasAliasing, hasElaborateAssign, isAssignable,
                        isStaticArray;
...
{
    //11853
    import stdd.traits : isAssignable;
...
{
    import stdd.exception : doesPointTo, mayPointTo;
    static struct S2
    {
        union
        {
...
Params:
    r1 = an $(REF_ALTTEXT input _range, isInputRange, stdd,_range,primitives)
         with swappable elements
    r2 = an $(REF_ALTTEXT input _range, isInputRange, stdd,_range,primitives)
         with swappable elements
