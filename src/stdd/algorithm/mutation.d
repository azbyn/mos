...
@safe unittest
{
    import stdd.algorithm, std.range;
    int[] src = [1, 2, 4];
    int[] dest = [0, 0, 0, 0, 0];
...
a `filter` $(REF_ALTTEXT forward range, isForwardRange, stdd,range,primitives)
can be provided. The length of `filler` and `range` do not need to match, but
`filler` must not be empty.

Params:
        range = An
                $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
...
$(REF_ALTTEXT bidirectional range, isBidirectionalRange, stdd,range,primitives) `range` by removing
...
Params:
    r1 = an $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
         with swappable elements
    r2 = an $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
         with swappable elements
