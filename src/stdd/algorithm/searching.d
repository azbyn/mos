...
$(T2 find,
        ...
        (For binary search refer to $(REF SortedRange, stdd,range).))
$(T2 findAdjacent,
...
Source: $(PHOBOSSRC stdd/algorithm/_searching.d)

Macros:
T2=$(TR $(TDNW $(LREF $1)) $(TD $+))
 */
module stdd.algorithm.searching;
...
import stdd.traits;
// FIXME
import stdd.typecons; // : Tuple, Flag, Yes, No;
...
Params:
    ...
    r1 = A $(REF_ALTTEXT forward range, isForwardRange, stdd,range,primitives) of
        elements.

    r2 = An $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives) of
        elements.
...
if (isNarrowString!R1 && isNarrowString!R2)
{
    ...
    {
        import stdd.utf : stride, UTFException;
...
/++
    ...
    $(REF_ALTTEXT forward range, isForwardRange, stdd,range,primitives)
    until the given predicate is true for one of the given `needles`.

    Params:
        pred = The predicate for determining when to stop counting.
        haystack = The
            $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives) to be
            counted.
    ...
    See_Also: $(REF indexOf, stdd,string)
  +/
...
@safe unittest
{
    ...
    assert(countUntil!(stdd.uni.isWhite)("hello world") == 5);
    assert(countUntil!(stdd.ascii.isDigit)("hello world") == -1);
...
@safe pure unittest
{
    import stdd.range : enumerate, iota;
...
// find
/**
Finds an individual element in an $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives).
...
The predicate is passed to $(REF binaryFun, stdd, functional), and can either accept a
string, or any callable that can be executed via `pred(element, element)`.

To _find the last occurrence of `needle` in a
$(REF_ALTTEXT bidirectional, isBidirectionalRange, stdd,range,primitives) `haystack`,
call `find(retro(haystack), needle)`. See $(REF retro, stdd,range).

If no `needle` is provided, `pred(haystack.front)` will be evaluated on each
element of the input range.

If `input` is a $(REF_ALTTEXT forward range, isForwardRange, stdd,range,primitives),
`needle` can be a $(REF_ALTTEXT forward range, isForwardRange, stdd,range,primitives) too.
...
Complexity:
    ...
    advantage of $(REF_ALTTEXT bidirectional, isBidirectionalRange, stdd,range,primitives)
    or $(REF_ALTTEXT random access, isRandomAccess, stdd,range,primitives)
...
Params:
    ...
    haystack = The $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
               searched in.
        ...
        && is(typeof(binaryFun!pred(haystack.front, needle.front)) : bool))
{
    ...
    else // static if (hasSlicing!R1 && isBidirectionalRange!R2)
    {
        ...
        // 8829 enhancement
        import stdd.algorithm.comparison : mismatch;
        import stdd.range : SortedRange;
        static if (is(R1 == R2)
...
@safe unittest
{
    import stdd.algorithm.internal : rndstuff;
    import stdd.meta : AliasSeq;
    import stdd.uni : toUpper;
...
See_Also:
$(REF among, stdd,algorithm,comparison) for checking a value against multiple possibilities.
 +/
...
Params:
    ...
    r = A $(REF_ALTTEXT forward range, isForwardRange, stdd,range,primitives) to
        search in.
...
Params:
    ...
    seq = The $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives) to
        search.
    choices = A $(REF_ALTTEXT forward range, isForwardRange, stdd,range,primitives)
        of possible choices.
...
See_Also: $(LREF find), $(REF stdd,algorithm,comparison,among)
*/
 ...
 * Params:
 *  haystack = The
 *   $(REF_ALTTEXT forward range, isForwardRange, stdd,range,primitives) to search
 *   in.
 *  needle = The
 *   $(REF_ALTTEXT forward range, isForwardRange, stdd,range,primitives) to search
 *   for.
...
$(REF_ALTTEXT forward range, isForwardRange, stdd,range,primitives) and
...
/// Returning a subtype of $(REF Tuple, stdd,typecons) enables
...
/// Use $(REF only, stdd,range) to find single elements:
@safe pure nothrow unittest
{
...
Params:
    pred = The ordering predicate to use to determine the extremum (minimum
        or maximum).
    range = The $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives) to count.
...
See_Also: $(REF min, stdd,algorithm,comparison), $(LREF minIndex), $(LREF minElement), $(LREF minPos)
 */
    ...
    is(typeof(binaryFun!pred(range.front, range.front))))
{
    ...
    {
        import stdd.algorithm.internal : addressOf;
...
See_Also:
    ...
    $(LREF maxElement), $(REF min, stdd,algorithm,comparison), $(LREF minCount),
...
See_Also:
    ...
    $(LREF minElement), $(REF max, stdd,algorithm,comparison), $(LREF maxCount),
...
Params:
    pred = The ordering predicate to use to determine the extremum (minimum or
        maximum) element.
    range = The $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives) to search.
...
See_Also:
    $(REF max, stdd,algorithm,comparison), $(LREF minCount), $(LREF minIndex), $(LREF minElement)
*/
...
Params:
    ...
    range = The $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
    to search.
...
See_Also:
    $(LREF maxIndex), $(REF min, stdd,algorithm,comparison), $(LREF minCount), $(LREF minElement), $(LREF minPos)
 */
...
See_Also:
    $(LREF minIndex), $(REF max, stdd,algorithm,comparison), $(LREF maxCount), $(LREF maxElement), $(LREF maxPos)
 */
...
{
    /**
    r1 = The $(REF_ALTTEXT forward range, isForwardRange, stdd,range,primitives) to
        move forward.
    r2 = The $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
...
$(REF_ALTTEXT input range, isInputRange, stdd,range,primitives) starts with (one
...
Params:
    ...
    range = The $(REF_ALTTEXT input _range, isInputRange, stdd,_range,primitives)
    to iterate over.
...
Returns:
    An $(REF_ALTTEXT input _range, isInputRange, stdd,_range,primitives) that
    iterates over the original range's elements, but ends when the specified
    predicate becomes true. If the original range is a
    $(REF_ALTTEXT forward _range, isForwardRange, stdd,_range,primitives) or
...
{
    import stdd.algorithm.comparison : among, equal;
    auto s = "hello how\nare you";
