...
Source: $(PHOBOSSRC stdd/algorithm/_setops.d)

Macros:
T2=$(TR $(TDNW $(LREF $1)) $(TD $+))
 */
module stdd.algorithm.setops;
...
import stdd.traits;
// FIXME
import stdd.meta; // : AliasSeq, staticMap, allSatisfy, anySatisfy;

import stdd.algorithm.sorting; // : Merge;
...
other an $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives).
...
Returns:
    A forward range of $(REF Tuple, stdd,typecons) representing elements of the
...
{
    import stdd.algorithm.iteration : map, joiner;

    static if (isInfinite!R1 && isInfinite!R2)
    {
        static if (isForwardRange!R1 && isForwardRange!R2)
        {
            import stdd.range : zip, repeat, take, chain, sequence;
...
{
    import stdd.range.primitives : isForwardRange;
    int[2] A = [1,2];
...
/**
Given a range of sorted $(REF_ALTTEXT forward ranges, isForwardRange, stdd,range,primitives)
...
import stdd.algorithm.sorting : SortOutput; // FIXME
...
Params:
    less = The predicate the ranges are sorted by.
    ror = A range of $(REF_ALTTEXT forward ranges, isForwardRange, stdd,range,primitives)
    sorted by `less`.
    tgt = The target range to copy common elements to.
...
{
    import stdd.algorithm.iteration : group;
    import stdd.algorithm.sorting : topNCopy;
...
@system unittest
{
    ...
    import stdd.typecons : tuple, Tuple, Yes;
...
struct MultiwayMerge(alias less, RangeOfRanges)
{
    ...
    {
        import stdd.algorithm.mutation : remove, SwapStrategy;
...
$(REF_ALTTEXT input ranges, isInputRange, stdd,range,primitives) are passed
...
See_Also:
   $(REF merge, stdd,algorithm,sorting)
 */
