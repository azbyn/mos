...
Source: $(PHOBOSSRC stdd/_array.d)
*/
module stdd.array;
...
public import stdd.range.primitives : save, empty, popFront, popBack, front, back;
...
See_Also: $(REF Tuple, stdd,typecons), $(REF zip, stdd,range)
 */
...
Returns: A $(REF_ALTTEXT forward range, isForwardRange, stdd,_range,primitives)
...
@safe pure nothrow unittest // bugzilla 9836
{
    ...
    stdd.array.popFront(w); // should work
...
/++
    ...
    Params:
        ...
        stuff = An $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives),
...
/**
Params:
    s = an $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
    or a dynamic array
...
See_Also:
    For a lazy version, refer to $(REF repeat, stdd,range).
 */
...
if (isDynamicArray!S)
{
    ...
    // Optimization for return join(stdd.range.repeat(s, n));
    if (n == 0)
        return RetType.init;
    if (n == 1)
...
{
    import stdd.range : repeat;
    return join(stdd.range.repeat(s, n));
}
...
The `range` must be a $(REF_ALTTEXT forward _range, isForwardRange, stdd,_range,primitives).
...
See_Also:
$(REF splitter, stdd,algorithm,iteration) for a lazy version without allocating memory.

$(REF splitter, stdd,regex) for a version that splits using a regular
...
/++
   ...
   Params:
        ror = An $(REF_ALTTEXT input range, isInputRange, stdd,range,primitives)
        of input ranges
   ...
   See_Also:
        For a lazy version, see $(REF joiner, stdd,algorithm,iteration)
  +/
...
/++
    ...
    Params:
        sink = an $(REF_ALTTEXT output range, isOutputRange, stdd,range,primitives)
    ...
    See_Also:
        $(REF substitute, stdd,algorithm,iteration) for a lazy replace.
 +/
    ...
    isForwardRange!R2 && is(typeof(appender!(E[])().put(to[0 .. 1]))))
{
    ...
    static if (isSomeString!(E[]))
    {
        import stdd.string : lastIndexOf;
