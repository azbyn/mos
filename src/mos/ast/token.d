...
alias Token = DToken;
mosstring mosstr(const ref Token t) {
    return (cast(moschar*) t.str)[0 .. t.len].idup();
}

mosstring mosstr(const Token* t) {
    return (cast(moschar*) t.str)[0 .. t.len].idup();
}
mosstring toStr(const Token* t) {
    import mos.misc;
    final switch (t.type) {
    ...
    case TT.Identifier: return t.mosstr;
    case TT.Number: return t.mosstr;
    case TT.String: return mosformat!`"%s"`(t.mosstr);
    ...
    size_t len;
    t.toString(&ptr, &len);
    auto res = (cast(moschar*) t.str)[0 .. t.len].idup();
    free(ptr);
    return res;*/
}
mosstring toStr(const ref Token t) {
    return toStr(&t);
}

mosstring unaryFunctionName(TT type) {
    switch (type) {
        // dfmt off
...
mosstring binaryFunctionName(TT type) {
    final switch (type) {
        static foreach (a; typeDatas)
