module mos.ast.ast_node;

import stdd.format;
import mos.misc : FuncType;
...
class AstNode {
    ...
    struct Lambda {
        ...
        bool isVariadic;
        AstNode body_;
        this(mosstring[] params, AstNode body_, bool isVariadic = false) {
            this.params = params;
        ...
        this(mosstring[] captures, mosstring[] params, AstNode body_, bool isVariadic = false) {
            this.params = params;
    ...
    struct Module {
        mosstring name;
        AstNode[mosstring] members;
    }

    struct Struct {
        mosstring name;
        AstNode ctor;
        AstNode[mosstring] statics;
        AstNode[mosstring] instance;
    }
    ...
    private void freeVarsTail(ref mosstring[] res, mosstring[] bound) {
        ...
        void fv_(AstNode[mosstring] nodes) {
    ...
    mosstring[] freeVars(mosstring[] bound = []) {
        mosstring[] res;
    ...
    string toString(int level) {
        ...
        string str_(string name, AstNode[mosstring] args) {
            string r = format!"%s  =%s:\n"(indent, name);
            foreach (name, a; args) {
