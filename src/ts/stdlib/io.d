...
@tsexport Module Attr() { return attrEnum; }

@tsexport @tsmodule abstract class Console {
static:
    @tsexport {
        void setAttr() {
            tsattr();
        ...
        @tsget tsint Flags() { return tsGetFlags(); }
        @tsset tsint Flags(tsint flags) { tsSetFlags(cast(ubyte) flags); return flags; }

        @tsget tsint Fg() { return tsGetFg(); }
        @tsset tsint Fg(tsint val) { tsSetFg(cast(uint) val); return val; }

        @tsget tsint Bg() { return tsGetBg(); }
        @tsset tsint Bg(tsint val) { tsSetBg(cast(uint) val); return val; }
        void clear() {tsclear(); }
        void puts(tsstring s) { tsputs(s); }
        void putnl() { tsputnl(); }
    }
}
@tsexport @tsmodule abstract class Color {
static:
