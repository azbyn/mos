/**
$(D stdd._parallelism) implements high-level primitives for SMP _parallelism.
These include parallel foreach, parallel reduce, parallel eager map, pipelining
and future/promise _parallelism.  $(D stdd._parallelism) is recommended when the
...
$(D stdd._parallelism) is based on the concept of a $(D Task).  A $(D Task) is an
...
Source:    $(PHOBOSSRC stdd/_parallelism.d)
Author:  David Simcha
Copyright:  Copyright (c) 2009-2011, David Simcha.
License:    $(HTTP boost.org/LICENSE_1_0.txt, Boost License 1.0)
*/
module stdd.parallelism;
...
unittest
{
    ...
    import stdd.parallelism : taskPool;

    // Parallel reduce can be combined with
    // stdd.algorithm.iteration.map to interesting effect.
    // The following example (thanks to Russel Winder)
    // calculates pi by quadrature  using
    // stdd.algorithm.map and TaskPool.reduce.
...
else version(linux)
{
    shared static this()
    {
        import core.sys.posix.unistdd : _SC_NPROCESSORS_ONLN, sysconf;
...
// Should be private, but stdd.algorithm.reduce is used in the zero-thread case
...
The $(REF task, stdd,parallelism) and $(REF scopedTask, std,parallelism) functions can
...
struct Task(alias fun, Args...)
{
    ...
    */
    ...
    static if (__traits(isSame, fun, run))
    {
        ...
        {
            // BUG:  Should check this for delegates too, but stdd.traits
    ...
    */
    ...
    $(D core.thread.Thread.priority). See $(REF task, stdd,parallelism) for
    usage example.
    */
...
$(REF TaskPool, stdd,parallelism).  A globally accessible instance of
$(D TaskPool) is provided by $(REF taskPool, stdd,parallelism).
...
// of an alias to represent stdd.file.read.
import stdd.file;
...
2.  $(D F) must not have any unshared aliasing as defined by
    $(REF hasUnsharedAliasing, stdd,traits).  This means it
...
1.  It distinguishes stdd.parallelism threads from other threads so that
    the stdd.parallelism daemon threads can be terminated.
...
available via the $(REF taskPool, stdd,parallelism) property.
...
public:
    ...
    */
    ...
    template amap(functions...)
    {
        ...
        $(REF map, stdd,algorithm,iteration).
    ...
    template map(functions...)
    {
        ...
        auto dupedLines = stdd.algorithm.map!"a.idup"(lineRange);
        ...
        */
        ...
        if (isInputRange!S)
        {
            ...
            public:
                ...
                static if (stdd.range.isInfinite!S)
                {
    ...
    void main()
    {
        ...
        auto duped = stdd.algorithm.map!"a.idup"(lines);
    ...
    */
    ...
    template reduce(functions...)
    {
        ...
        usage is similar to $(REF _reduce, stdd,algorithm,iteration).  This
        ...
        relative to $(REF _reduce, stdd,algorithm,iteration), especially for
        ...
        from those generated by $(REF _reduce, stdd,algorithm,iteration) or
        ...
        // Using stdd.algorithm.reduce instead:  181 milliseconds
        auto nums = iota(10_000_000.0f);
        auto sumSquares = taskPool.reduce!"a + b"(
            0.0, stdd.algorithm.map!"a * a"(nums)
        );
        ---
            ...
            else
            {
                ...
                if (!ptr)
                {
                    throw new OutOfMemoryError(
                        "Out of memory in stdd.parallelism."
                    );
                }
    ...
    import stdd.conv, std.range, std.numeric, std.stdio;
    ...
    */
    ...
    $(REF hasUnsharedAliasing, stdd,traits) is false for the $(D Task)'s
...
)
{
    ...
    else
    {
        ...
        if (!ptr) throw new OutOfMemoryError("Out of memory in stdd.parallelism.");
...
unittest
{
    ...
    auto indexed = stdd.range.indexed(toIndex, [3, 1, 4, 0, 2]);
    ...
    immutable serialAns = stdd.algorithm.reduce!"a + b"(iota(1000));
    ...
    assert(equal(
               ...
               stdd.algorithm.map!"a * a"(iota(30_000_001))
           ));
    ...
    assert(
        ...
        reduce!"a + b"(0UL,
                       stdd.algorithm.map!"a * a"(iota(3_000_001))
                      )
    );
    ...
    auto myTask = task!(stdd.math.abs)(-1);
    taskPool.put(myTask);
...
unittest
{
    // this test was in stdd.range, but caused cycles.
