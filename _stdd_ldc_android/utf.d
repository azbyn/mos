 ...
 * using ranges with `byCodeUnit` can be `nothrow` while $(REF front, stdd,range,primitives)
 ...
 *
 *      Otherwise, an input range with a length if $(REF isAggregateType, stdd,traits)
 *      is `true` for `R`. Otherwise, this returns a finite random access range
 *      with slicing.
 * See_Also:
 *      Refer to the $(MREF stdd, uni) docs for a reference on Unicode terminology.
 *
