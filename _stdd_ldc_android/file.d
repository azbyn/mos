...
$(MREF stdd, stdio) for opening files and manipulating them via handles,
and module $(MREF stdd, path) for manipulating path strings.

License:   $(HTTP boost.org/LICENSE_1_0.txt, Boost License 1.0).
Authors:   $(HTTP digitalmars.com, Walter Bright),
           $(HTTP erdani.org, Andrei Alexandrescu),
           Jonathan M Davis
Source:    $(PHOBOSSRC stdd/_file.d)
 */
module stdd.file;
...
import stdd.internal.cstring;

version (Windows)
{
    import core.sys.windows.windows, stdd.windows.syserror;
}
else version (Posix)
{
    import core.sys.posix.dirent, core.sys.posix.fcntl, core.sys.posix.sys.stat,
        core.sys.posix.sys.time, core.sys.posix.unistdd, core.sys.posix.utime;
}
else
...
@property string deleteme() @safe
{
    ...
    import stdd.process : thisProcessID;
...
version (Posix) private void[] readImpl(const(char)[] name, const(FSChar)* namez, size_t upTo = size_t.max) @trusted
{
    ...
    import stdd.array : uninitializedArray;
    ...
    cenforce(fd != -1, name);
    scope(exit) core.sys.posix.unistdd.close(fd);
    ...
    for (;;)
    {
        immutable actual = core.sys.posix.unistdd.read(fd, result.ptr + size,
...
version (linux) @safe unittest
{
    ...
    auto s = stdd.file.readText("/proc/sys/kernel/osrelease");
    assert(s.length > 0);
    //writefln("'%s'", s);
}
...
/********************************************
Read and validates (using $(REF validate, stdd,utf)) a text file. $(D S)
...
See_also: $(REF toFile, stdd,stdio)
 */
        ...
        in void[] buffer, bool append) @trusted
{
    ...
    {
        scope(failure) core.sys.posix.unistdd.close(fd);
        ...
        while (sum != size)
        {
            ...
            const numwritten = core.sys.posix.unistdd.write(fd, buffer.ptr + sum, cnt);
            if (numwritten != cnt)
                break;
            sum += numwritten;
        }
    ...
    cenforce(core.sys.posix.unistdd.close(fd) == 0, name, namez);
}
...
else version(Windows)
{
            ...
            !isConvertibleToString!R)
    {
        with (getFileAttributesWin(name))
        {
            fileCreationTime = stdd.datetime.FILETIMEToSysTime(&ftCreationTime);
            fileAccessTime = stdd.datetime.FILETIMEToSysTime(&ftLastAccessTime);
            fileModificationTime = stdd.datetime.FILETIMEToSysTime(&ftLastWriteTime);
        }
    }
...
@safe unittest
{
    //stdd.process.system("echo a > deleteme") == 0 || assert(false);
    if (exists(deleteme))
        remove(deleteme);
...
@system unittest
{
    ...
    else version(Posix)
    {
        if (system_directory.exists)
        {
            ...
            core.sys.posix.unistdd.symlink(system_directory, symfile.ptr);
        ...
        if (system_file.exists)
        {
            ...
            core.sys.posix.unistdd.symlink(system_file, symfile.ptr);
...
Example:
--------------------
core.sys.posix.unistdd.symlink("/etc/fonts/fonts.conf", "/tmp/alink");
        ...
        !isConvertibleToString!R)
{
    ...
    else version(Posix)
    {
        static auto trustedChdir(const(FSChar)* pathz) @trusted
        {
            return core.sys.posix.unistdd.chdir(pathz) == 0;
        }
    }
    static if (isNarrowString!R && is(Unqual!(ElementEncodingType!R) == char))
...
{
    import stdd.path : dirName, baseName;
...
@system unittest
{
    ...
    {
        import stdd.path : buildPath, buildNormalizedPath;
        ...
        !isConvertibleToString!R)
{
    ...
    else version(Posix)
    {
        static auto trustedRmdir(const(FSChar)* pathz) @trusted
        {
            return core.sys.posix.unistdd.rmdir(pathz) == 0;
        }
    }
    static if (isNarrowString!R && is(Unqual!(ElementEncodingType!R) == char))
        alias pathStr = pathname;
            ...
            isConvertibleToString!RL))
{
    ...
    else
    {
        ...
        alias posixSymlink = core.sys.posix.unistdd.symlink;
        ...
        isConvertibleToString!R)
{
    ...
    else
    {
        ...
        alias posixReadlink = core.sys.posix.unistdd.readlink;
        enum bufferLen = 2048;
        enum maxCodeUnits = 6;
...
version(Posix) @system unittest // input range of dchars
{
    ...
    write(deleteme ~ "/f", "");
    import stdd.range.interfaces : InputRange, inputRangeObject;
    import stdd.utf : byChar;
...
else version (Solaris) string getcwd()
{
    ...
    auto p = cenforce(core.sys.posix.unistdd.getcwd(null, BUF_SIZE),
            "cannot get cwd");
...
{
    auto p = cenforce(core.sys.posix.unistdd.getcwd(null, 0),
            "cannot get cwd");
    scope(exit) core.stdc.stdlib.free(p);
    return p[0 .. core.stdc.string.strlen(p)].idup;
}
...
@trusted string thisExePath ()
{
    ...
    {
        import stdd.exception : errnoEnforce, assumeUnique;
        enum
        {
            CTL_KERN = 1,
            KERN_PROC = 14,
    ...
    {
        import core.sys.posix.unistdd : getpid;
    ...
    public:
        ...
        this(string path)
        {
            ...
            with (getFileAttributesWin(path))
            {
                ...
                _timeCreated = stdd.datetime.FILETIMEToSysTime(&ftCreationTime);
                _timeLastAccessed = stdd.datetime.FILETIMEToSysTime(&ftLastAccessTime);
                _timeLastModified = stdd.datetime.FILETIMEToSysTime(&ftLastWriteTime);
        ...
        private this(string path, in WIN32_FIND_DATAW *fd)
        {
            ...
            _timeCreated = stdd.datetime.FILETIMEToSysTime(&fd.ftCreationTime);
            _timeLastAccessed = stdd.datetime.FILETIMEToSysTime(&fd.ftLastAccessTime);
            _timeLastModified = stdd.datetime.FILETIMEToSysTime(&fd.ftLastWriteTime);
...
@system unittest
{
    ...
    else version(Posix)
    {
        ...
        if (system_directory.exists)
        {
            ...
            symfile.remove();
            core.sys.posix.unistdd.symlink((deleteme ~ "_broken_symlink\0").ptr, symfile.ptr);
        ...
        PreserveAttributes preserve) @trusted
{
    ...
    else version(Posix)
    {
        ...
        {
            scope(failure) core.sys.posix.unistdd.close(fdw);
        ...
        scope(failure) core.stdc.stdio.remove(toz);
        {
            ...
            for (auto size = statbufr.st_size; size; )
            {
                ...
                cenforce(
                    core.sys.posix.unistdd.read(fdr, buf, toxfer) == toxfer
                    && core.sys.posix.unistdd.write(fdw, buf, toxfer) == toxfer,
                    f, fromz);
                assert(size >= toxfer);
                size -= toxfer;
            }
        ...
        cenforce(core.sys.posix.unistdd.close(fdw) != -1, f, fromz);
...
@safe unittest
{
    import stdd.algorithm, std.file; // issue 14817
    auto t1 = deleteme, t2 = deleteme~"2";
    scope(exit) foreach (t; [t1, t2]) if (t.exists) t.remove();
    write(t1, "11");
    copy(t1, t2);
...
{
    import stdd.exception : enforce, collectException;
    import stdd.process : executeShell;
    collectException(rmdirRecurse(deleteme));
    auto d = deleteme~"/a/b/c/d/e/f/g";
    enforce(collectException(mkdir(d)));
    mkdirRecurse(d);
    core.sys.posix.unistdd.symlink((deleteme~"/a/b/c\0").ptr,
            (deleteme~"/link\0").ptr);
...
{
    import stdd.array : Appender, appender;
    SpanMode _mode;
...
// Hook it up with stdd.parallelism to compile them all in parallel:
foreach (d; parallel(dFiles, 1)) //passes by 1 file to each thread
{
    string cmd = "dmd -c "  ~ d.name;
    writeln(cmd);
    stdd.process.system(cmd);
}
--------------------
 +/
...
/// Duplicate functionality of D1's $(D stdd.file.listdir()):
@safe unittest
{
    string[] listdir(string pathname)
    {
        ...
        return stdd.file.dirEntries(pathname, SpanMode.shallow)
            .filter!(a => a.isFile)
            .map!(a => stdd.path.baseName(a.name))
            .array;
    }
...
@system unittest
{
    ...
    import stdd.array : array;
    import stdd.conv : to;
    import stdd.path : dirEntries, buildPath, absolutePath;
    import stdd.process : thisProcessID;
    ...
    version(Android)
        ...
        string testdir = "deleteme.dmd.unittest.stdd.file" ~ to!string(thisProcessID); // needs to be relative
    ...
    bool followSymlink = true)
{
    ...
    import stdd.path : globMatch, baseName;
...
@system unittest
{
    ...
    version (Posix)
    {
        core.sys.posix.unistdd.symlink((dpath ~ '\0').ptr, (sdpath ~ '\0').ptr);
        core.sys.posix.unistdd.symlink((fpath ~ '\0').ptr, (sfpath ~ '\0').ptr);
    }
 ...
 * $(REF Tuple, stdd,typecons) of values depending on the length of `Types`.
 * The lines are parsed using the specified format string. The format string is
 * passed to $(REF formattedRead, stdd,format), and therefore must conform to the
 ...
 *     array of $(REF Tuple, stdd,typecons)s.
 *
 * Throws:
 ...
 *     to $(REF formattedRead, stdd,format). Meaning that no empty lines or lines
...
string tempDir() @trusted
{
    ...
    if (cache is null)
    {
        ...
        {
            import stdd.process : environment;
