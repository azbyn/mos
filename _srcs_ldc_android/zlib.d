...
private import stdd.stdio;
private import stdd.random;

@system unittest // by Dave
{
    debug(zlib) writeln("stdd.zlib.unittest");

    bool CompressThenUncompress (void[] src)
    {
        ubyte[] dst = stdd.zlib.compress(src);
        ...
        uncompressedBuf = cast(ubyte[]) stdd.zlib.uncompress(dst);
    ...
    debug(zlib) writefln("PASSED stdd.zlib.unittest");
}
